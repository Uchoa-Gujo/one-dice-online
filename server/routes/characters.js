  ...
  if (isPlainObject(existing) && isPlainObject(incoming)) {
    for (const [key, value] of Object.entries(existing)) {
      if (isPlainObject(value) && isPlainObject(incoming[key])) {
        out[key] = mergeObjectsPreservingExisting(value, incoming[key]);
      } else if ((incoming[key] === undefined || incoming[key] === null || incoming[key] === '') && value !== undefined && value !== null && value !== '') {
        out[key] = value;
      }
    }
  }
...
function skillHasMeaning(value) {
  if (!isPlainObject(value)) return false;
  if (value.trained === true || value.disadvantage === true) return true;
  if (Number(value.bonus || 0) !== 0) return true;
  if (typeof value.notes === 'string' && value.notes.trim()) return true;
  return false;
}

function skillLooksDefault(value) {
  if (!isPlainObject(value)) return true;
  return value.trained !== true && value.disadvantage !== true && Number(value.bonus || 0) === 0 && !(typeof value.notes === 'string' && value.notes.trim());
}

function mergeSkillsSafely(existingSkills = {}, incomingSkills = {}, presentSkills = null) {
  const existing = isPlainObject(existingSkills) ? existingSkills : {};
  const incoming = isPlainObject(incomingSkills) ? incomingSkills : {};
  const out = mergeObjectsPreservingExisting(existing, incoming);
  const hasPresentList = presentSkills instanceof Set && presentSkills.size > 0;

  for (const [name, oldSkill] of Object.entries(existing)) {
    const nextSkill = incoming[name];
    if (hasPresentList && !presentSkills.has(name)) {
      out[name] = oldSkill;
      continue;
    }
    ...
    // Payloads antigos criavam todas as perícias como falso/0 quando a aba não estava carregada.
    // Quando a perícia antiga tinha valor real, não deixamos esse reset acidental apagar.
    if (!hasPresentList && skillHasMeaning(oldSkill) && skillLooksDefault(nextSkill)) {
      out[name] = oldSkill;
    }
  }
  return out;
...
function arrayLooksBlank(value) {
  ...
  return value.every(item => {
    ...
    const text = Object.entries(item)
      ...
      .map(([, v]) => String(v ?? '').trim())
      .join('');
    return !text || /^(0|PE|Padrão|Habilidade|Item)+$/i.test(text);
  });
}

function stripInternalSaveMarkers(data) {
  if (!isPlainObject(data)) return data;
  const out = { ...data };
  delete out._presentSkills;
  delete out._saveSourceVersion;
  return out;
}

function mergeCharacterDataSafely(existingData = {}, incomingData = {}) {
  const existing = isPlainObject(existingData) ? existingData : {};
  const incomingRaw = isPlainObject(incomingData) ? incomingData : {};
  const presentSkills = Array.isArray(incomingRaw._presentSkills) ? new Set(incomingRaw._presentSkills.map(String)) : null;
  const incoming = stripInternalSaveMarkers(incomingRaw);
  const out = { ...existing, ...incoming };

  const protectedArrays = [
    'inventoryItems', 'blockInventory', 'abilities', 'spells', 'attacks',
    'conditions', 'transformations', 'dropItems'
  ];
  const protectedObjects = [
    'resistances', 'attrs', 'caster', 'obsIcons', 'portraitCrop', 'settings'
  ];
  const protectedTexts = ['abilitiesNotes', 'equipmentNotes'];

  for (const key of protectedArrays) {
    if (hasUsefulArray(existing[key]) && (!hasUsefulArray(incoming[key]) || arrayLooksBlank(incoming[key]))) out[key] = existing[key];
  }

  out.skills = mergeSkillsSafely(existing.skills || {}, incoming.skills || {}, presentSkills);

  for (const key of protectedObjects) {
    if (hasUsefulObject(existing[key]) || hasUsefulObject(incoming[key])) {
      out[key] = mergeObjectsPreservingExisting(existing[key] || {}, incoming[key] || {});
    }
  }
  for (const key of protectedTexts) {
    if (typeof existing[key] === 'string' && existing[key].trim() && !(typeof incoming[key] === 'string' && incoming[key].trim())) {
      out[key] = existing[key];
    }
  }

  return normalizeCharacterData(out);
}
...
router.post('/', async (req, res) => {
  ...
  if ((limit.rows[0]?.total || 0) >= 20) return res.status(403).json({ error: 'Limite de 20 personagens por conta atingido.' });
...
router.put('/:id', async (req, res) => {
  ...
  const previous = await query('select data from characters where id = $1 and owner_id = $2', [id, req.user.id]);
