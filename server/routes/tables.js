...
router.delete('/:id', async (req, res) => {
  const tableId = req.params.id;
  const ownerCheck = await query('select id from tables where id = $1 and owner_id = $2', [tableId, req.user.id]);
  if (!ownerCheck.rowCount) return res.status(403).json({ error: 'Somente o dono pode excluir a mesa.' });

  // V143: desvincula explicitamente todas as fichas antes de apagar a campanha.
  // A FK já remove membros por cascade, mas esta etapa evita estados intermediários
  // e deixa claro para clientes/socket que não há mais character_id vinculado.
  await query('update table_members set character_id = null, updated_at = now() where table_id = $1', [tableId]);
  emitTable(req, tableId, 'member:updated', { reason: 'table-delete-unlink' });

  await query('delete from chat_messages where table_id = $1', [tableId]);
  await query('delete from table_members where table_id = $1', [tableId]);
  await query('delete from tables where id = $1 and owner_id = $2', [tableId, req.user.id]);
  emitTable(req, tableId, 'table:deleted', { reason: 'deleted' });
  res.json({ ok: true });
});
