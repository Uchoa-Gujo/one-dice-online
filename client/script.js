...
(function od139EarlyGuards(){
  'use strict';
  window.ONE_DICE_CLIENT_VERSION = '1.63.0';
...
/* =========================
   V163 - Redesign + URLs legíveis
   - Camada final segura: melhora UI, breadcrumbs e rotas sem reescrever regras da ficha.
   - Rotas passam a usar nomes: /personagem/nome--id e /mesa/nome--id/personagem/nome--id.
========================= */
(function od163RedesignAndPrettyRoutes(){
  'use strict';
  if (window.__od163RedesignInstalled) return;
  window.__od163RedesignInstalled = true;
  window.ONE_DICE_CLIENT_VERSION = '1.63.0';

  const VERSION = '1.63.0';
  const ROUTE_FLAG = 'od163Route';
  let applyingPrettyRoute = false;
  let routeTimer = null;
  let chipTimer = null;

  function $(id){ return document.getElementById(id); }
  function list(key){
    try { return typeof get === 'function' ? (get(key, []) || []) : []; } catch (_) { return []; }
  }
  function chars(){ return list(STORAGE?.characters); }
  function campaigns(){ try { return typeof getCampaigns === 'function' ? (getCampaigns() || []) : list(STORAGE?.campaigns); } catch (_) { return []; } }
  function members(){ try { return typeof getMembers === 'function' ? (getMembers() || []) : list(STORAGE?.members); } catch (_) { return []; } }
  function currentCharSafe(){ try { return typeof currentChar === 'function' ? currentChar() : chars().find(c => String(c.id) === String(currentCharacterId)); } catch (_) { return null; } }
  function activeCampaignSafe(){
    try {
      const id = typeof currentCampaignId !== 'undefined' ? currentCampaignId : null;
      return campaigns().find(c => String(c.id) === String(id)) || (typeof activeCampaign === 'function' ? activeCampaign() : null);
    } catch (_) { return null; }
  }
  ...
  function slugify(value, fallback = 'one-dice'){
  ...
  function packId(id){ return encodeURIComponent(String(id || '').trim()); }
  function unpackSlug(segment){
    const raw = decodeURIComponent(String(segment || ''));
    const marker = raw.lastIndexOf('--');
    return marker >= 0 ? raw.slice(marker + 2) : raw;
  }
  function tabName(){ return document.querySelector('.sheet-tab.active')?.dataset?.tab || 'resumo'; }
  function tabLabel(tab){
    const found = document.querySelector(`.sheet-tab[data-tab="${CSS.escape(tab || '')}"]`);
    return text(found?.textContent) || text(tab) || 'Resumo';
  }
  function isAppActive(){ return $('app-screen')?.classList.contains('active'); }
  ...
  function charPath(char = currentCharSafe(), tab = tabName()){
    if (!char?.id) return '/personagens';
    const suffix = tab ? `?aba=${encodeURIComponent(tab)}` : '';
    return `/personagem/${slugify(char.name || char.nome || 'ficha')}--${packId(char.id)}${suffix}`;
  }
  function tablePath(campaign = activeCampaignSafe()){
    if (!campaign?.id) return '/campanhas';
    return `/mesa/${slugify(campaign.name || campaign.title || 'mesa')}--${packId(campaign.id)}`;
  }
  function tableCharPath(campaign = activeCampaignSafe(), char = currentCharSafe(), tab = tabName()){
    if (!campaign?.id) return charPath(char, tab);
    if (!char?.id) return tablePath(campaign);
    return `/mesa/${slugify(campaign.name || campaign.title || 'mesa')}--${packId(campaign.id)}/personagem/${slugify(char.name || char.nome || 'ficha')}--${packId(char.id)}?aba=${encodeURIComponent(tab || 'resumo')}`;
  }
  function desiredPath(){
    if (isAuthActive()) return '/entrar';
    if (isSessionsActive()) {
      const tab = localStorage.getItem('od71_tab') || '';
      if (tab === 'characters') return '/personagens';
      if (tab === 'campaigns') return '/campanhas';
      return '/inicio';
    }
    if (isAppActive()) {
      const inAccount = !!(typeof accountSheetMode !== 'undefined' && accountSheetMode);
      if (inAccount || !currentCampaignId) return charPath();
      return tableCharPath();
    }
    return '/inicio';
  }
  function setRoute(path, replace = true){
    if (!path || applyingPrettyRoute) return;
    const current = location.pathname + location.search;
    if (current === path) return;
    try { history[replace ? 'replaceState' : 'pushState']({ [ROUTE_FLAG]: true, path }, '', path); } catch (_) {}
  }
  function scheduleRouteSync(delay = 90){
    clearTimeout(routeTimer);
    routeTimer = setTimeout(() => {
      try { setRoute(desiredPath(), true); updateTitleAndCrumbs(); } catch (_) {}
    }, delay);
  }
  function showChip(message){
    let chip = $('od163-route-chip');
    if (!chip) {
      chip = document.createElement('div');
      chip.id = 'od163-route-chip';
      chip.className = 'od163-route-chip';
      document.body.appendChild(chip);
    }
    chip.textContent = message;
    chip.classList.add('active');
    clearTimeout(chipTimer);
    chipTimer = setTimeout(() => chip.classList.remove('active'), 1300);
  }

  function ensureBreadcrumb(){
    if ($('od163-breadcrumb')) return $('od163-breadcrumb');
    const crumb = document.createElement('nav');
    crumb.id = 'od163-breadcrumb';
    crumb.className = 'od163-breadcrumb';
    crumb.setAttribute('aria-label', 'Caminho atual');
    const app = $('app-screen');
    if (app) app.insertBefore(crumb, app.querySelector('.app-grid') || app.firstChild);
    return crumb;
  }
  function updateTitleAndCrumbs(){
    try {
      const campaign = activeCampaignSafe();
      const char = currentCharSafe();
      const inAccount = !!(typeof accountSheetMode !== 'undefined' && accountSheetMode);
      let title = 'One Dice';
      const parts = [];
      if (isAuthActive()) { title = 'Entrar • One Dice'; parts.push('Entrar'); }
      else if (isSessionsActive()) {
        const tab = localStorage.getItem('od71_tab') || 'home';
        const label = tab === 'characters' ? 'Personagens' : tab === 'campaigns' ? 'Campanhas' : 'Início';
        title = `${label} • One Dice`;
        parts.push(label);
      } else if (isAppActive() && inAccount && char) {
        title = `${char.name || 'Ficha'} • One Dice`;
        parts.push('Personagens', `<strong>${esc(char.name || 'Ficha')}</strong>`);
      } else if (isAppActive() && campaign && char) {
        title = `${campaign.name || 'Mesa'} / ${char.name || 'Ficha'} • One Dice`;
        parts.push(`<strong>${esc(campaign.name || 'Mesa')}</strong>`, esc(char.name || 'Ficha'));
      } else if (isAppActive() && campaign) {
        title = `${campaign.name || 'Mesa'} • One Dice`;
        parts.push(`<strong>${esc(campaign.name || 'Mesa')}</strong>`);
      }
      document.title = title;
      const crumb = ensureBreadcrumb();
      if (crumb) {
        if (isAppActive()) {
          crumb.innerHTML = parts.map(p => `<span>${p}</span>`).join('');
  ...
  async function applyPrettyRouteFromLocation(){
    if (applyingPrettyRoute) return false;
    const parts = location.pathname.split('/').filter(Boolean).map(decodeURIComponent);
    if (!parts.length) return false;
    const first = parts[0];
    if (!['personagem','ficha','mesa','entrar','login'].includes(first)) return false;
    applyingPrettyRoute = true;
    try {
      if (first === 'entrar' || first === 'login') { if (typeof showAuth === 'function') showAuth(); return true; }
      if (!currentUser) {
        try { sessionStorage.setItem('od_pending_route', location.pathname + location.search); } catch (_) {}
        if (typeof showAuth === 'function') showAuth();
        return true;
      }
      const q = new URLSearchParams(location.search);
      const requestedTab = q.get('aba') || q.get('tab') || '';
      if (first === 'personagem' || first === 'ficha') {
        const id = unpackSlug(parts[1] || '');
        if (typeof initAccountCharacterEditor === 'function') initAccountCharacterEditor(id || null);
        if (requestedTab) setTimeout(() => document.querySelector(`.sheet-tab[data-tab="${CSS.escape(requestedTab)}"]`)?.click(), 80);
        setTimeout(() => { scheduleRouteSync(10); showChip('Ficha aberta fora da mesa'); }, 140);
        return true;
      }
      if (first === 'mesa') {
        const tableId = unpackSlug(parts[1] || '');
        const charMarker = parts.findIndex(p => p === 'personagem' || p === 'ficha');
        const charId = charMarker >= 0 ? unpackSlug(parts[charMarker + 1] || '') : '';
        if (tableId && typeof enterCampaign === 'function') await enterCampaign(tableId);
        if (charId && typeof loadCharacter === 'function') setTimeout(() => loadCharacter(charId), 120);
        if (requestedTab) setTimeout(() => document.querySelector(`.sheet-tab[data-tab="${CSS.escape(requestedTab)}"]`)?.click(), 180);
        setTimeout(() => scheduleRouteSync(10), 220);
        return true;
      }
    } catch (_) {
      return false;
    } finally {
      setTimeout(() => { applyingPrettyRoute = false; }, 260);
    }
    return false;
  }
  ...
  function polishStaticLabels(){
    try {
      const campaignBtn = $('campaign-character-btn');
      if (campaignBtn) campaignBtn.textContent = 'Ficha da Mesa';
      const logout = $('logout-btn');
      if (logout) logout.textContent = 'Sair';
      const history = $('toggle-history-btn');
      if (history) history.textContent = 'Histórico';
    } catch (_) {}
  }

  function installWrappers(){
    if (typeof showAuth === 'function' && !showAuth.__od163Route) {
      const base = showAuth;
      showAuth = function od163ShowAuth(){ const r = base.apply(this, arguments); scheduleRouteSync(30); return r; };
      showAuth.__od163Route = true;
      try { window.showAuth = showAuth; } catch (_) {}
    }
    if (typeof showSessions === 'function' && !showSessions.__od163Route) {
      const base = showSessions;
      showSessions = function od163ShowSessions(){ const r = base.apply(this, arguments); scheduleRouteSync(80); return r; };
      showSessions.__od163Route = true;
      try { window.showSessions = showSessions; } catch (_) {}
    }
    if (typeof initAccountCharacterEditor === 'function' && !initAccountCharacterEditor.__od163Route) {
      const base = initAccountCharacterEditor;
      initAccountCharacterEditor = function od163InitAccountCharacterEditor(){ const r = base.apply(this, arguments); scheduleRouteSync(120); return r; };
      initAccountCharacterEditor.__od163Route = true;
      try { window.initAccountCharacterEditor = initAccountCharacterEditor; } catch (_) {}
    }
    if (typeof enterCampaign === 'function' && !enterCampaign.__od163Route) {
      const base = enterCampaign;
      enterCampaign = async function od163EnterCampaign(){ const r = await base.apply(this, arguments); scheduleRouteSync(160); return r; };
      enterCampaign.__od163Route = true;
      try { window.enterCampaign = enterCampaign; } catch (_) {}
    }
    if (typeof initApp === 'function' && !initApp.__od163Route) {
      const base = initApp;
      initApp = function od163InitApp(){ const r = base.apply(this, arguments); scheduleRouteSync(140); return r; };
      initApp.__od163Route = true;
      try { window.initApp = initApp; } catch (_) {}
    }
    if (typeof loadCharacter === 'function' && !loadCharacter.__od163Route) {
      const base = loadCharacter;
      loadCharacter = function od163LoadCharacter(){ const r = base.apply(this, arguments); scheduleRouteSync(110); return r; };
      loadCharacter.__od163Route = true;
      try { window.loadCharacter = loadCharacter; } catch (_) {}
    }
  }

  document.addEventListener('click', event => {
    if (event.target.closest('.sheet-tab, [data-od71-tab], [data-od75-tab], [data-open-account-character], [data-od71-open-character], [data-edit-account-character], [data-enter-campaign], #back-to-sessions-btn, #campaign-character-btn')) {
      scheduleRouteSync(160);
    }
  }, true);

  window.addEventListener('popstate', () => setTimeout(applyPrettyRouteFromLocation, 30));

  function boot(){
    document.documentElement.dataset.od163 = '1';
    document.documentElement.dataset.odVersion = VERSION;
    polishStaticLabels();
    installWrappers();
    updateTitleAndCrumbs();
    setTimeout(polishStaticLabels, 300);
    setTimeout(installWrappers, 400);
    setTimeout(() => applyPrettyRouteFromLocation().then(applied => { if (!applied) scheduleRouteSync(30); }), 650);
    setTimeout(() => { scheduleRouteSync(30); updateTitleAndCrumbs(); }, 1300);
  }

  if (document.readyState === 'loading') document.addEventListener('DOMContentLoaded', boot, { once: true });
  else boot();

  window.od163 = { version: VERSION, slugify, charPath, tablePath, tableCharPath, syncRoute: scheduleRouteSync, applyPrettyRouteFromLocation };
})();
