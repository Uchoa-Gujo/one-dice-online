...
(function od139EarlyGuards(){
  'use strict';
  window.ONE_DICE_CLIENT_VERSION = '1.44.0';
...
/* =========================
   V143 - correções pós-limpeza: Defesa/Esquiva, sessão, backups e menu
   - Defesa e Esquiva ficam 100% independentes.
   - Remove rótulos "Manual" sob os campos.
   - Limpa vínculos órfãos quando mesa/campanha é apagada.
   - Limita restauração a 5 backups por ficha.
   - Guarda a barra rápida dentro do menu de três traços.
   - Torna o perfil da sessão acionável abrindo Configurações da Conta.
========================= */
(function od143UserFixes(){
  'use strict';
  if (window.__od143UserFixesInstalled) return;
  window.__od143UserFixesInstalled = true;
  window.ONE_DICE_CLIENT_VERSION = '1.44.0';
  ...
  function num(v, fallback = 10){
    const n = Number(v);
    return Number.isFinite(n) ? n : fallback;
  }
  function getSafe(key, fallback){
    try { return typeof get === 'function' ? get(key, fallback) : JSON.parse(localStorage.getItem(key) || JSON.stringify(fallback)); }
    catch (_) { return fallback; }
  }
  function setSafe(key, value){
    try { if (typeof set === 'function') set(key, value); else localStorage.setItem(key, JSON.stringify(value)); }
    catch (_) {}
  }
  function chars(){ return getSafe(STORAGE.characters, []); }
  function campaigns(){ return typeof getCampaigns === 'function' ? getCampaigns() : getSafe(STORAGE.campaigns, []); }
  function members(){ return typeof getMembers === 'function' ? getMembers() : getSafe(STORAGE.campaignMembers, []); }
  function saveMembers(list){ try { if (typeof setMembers === 'function') setMembers(list); else setSafe(STORAGE.campaignMembers, list); } catch (_) {} }
  function saveCampaigns(list){ try { if (typeof setCampaigns === 'function') setCampaigns(list); else setSafe(STORAGE.campaigns, list); } catch (_) {} }
  function activeChar(){
    try { return typeof currentChar === 'function' ? currentChar() : chars().find(c => String(c.id) === String(currentCharacterId || '')); }
    catch (_) { return null; }
  }
  function mutateChar(id, patcher){
    const list = chars();
    const idx = list.findIndex(c => String(c.id) === String(id));
    if (idx < 0) return null;
    patcher(list[idx]);
    list[idx].updatedAt = Date.now();
    setSafe(STORAGE.characters, list);
    return list[idx];
  }

  // 1 e 2 — Defesa/Esquiva independentes, sem texto "Manual" piscando.
  let lastDefenseInput = null;
  let lastDodgeInput = null;
  let lastKnownDodge = null;
  let lastKnownDefense = null;

  function rememberManualValues(){
    const char = activeChar();
    const defense = $('defense');
    const dodge = $('dodge');
    lastKnownDefense = defense ? defense.value : (char?.defense ?? lastKnownDefense);
    lastKnownDodge = dodge ? dodge.value : (char?.dodge ?? lastKnownDodge);
  }

  function blankManualNotes(){
    const defenseNote = $('defense-effective-note');
    const dodgeNote = $('dodge-formula-note');
    [defenseNote, dodgeNote].forEach(note => {
      if (!note) return;
      note.textContent = '';
  ...
  function forceManualDefenseDodge(source = ''){
    const char = activeChar();
    const defense = $('defense');
    const dodge = $('dodge');
    if (defense) {
      ...
      defense.title = 'Defesa manual independente.';
      if (document.activeElement !== defense && char) defense.value = num(char.defense, 10);
    }
    if (dodge) {
      dodge.removeAttribute('readonly');
      dodge.dataset.manualOnly = 'true';
      dodge.title = 'Esquiva manual independente.';
      if (document.activeElement !== dodge && char) dodge.value = num(char.dodge, 10);
    }
    blankManualNotes();
  }

  function saveDefenseOnly(value){
    const char = activeChar();
    if (!char?.id) return;
    const oldDodge = char.dodge;
    const next = mutateChar(char.id, c => {
      c.defense = num(value, c.defense ?? 10);
      c.dodge = oldDodge;
      c.dodgeManual = true;
      c.dodgeLocked = true;
    });
    if (next) {
      const dodge = $('dodge');
      if (dodge && document.activeElement !== dodge) dodge.value = num(next.dodge, 10);
      try { if (typeof od42ScheduleCharacterSave === 'function') od42ScheduleCharacterSave(next); } catch (_) {}
    }
  }

  function saveDodgeOnly(value){
    const char = activeChar();
    if (!char?.id) return;
    const oldDefense = char.defense;
    const next = mutateChar(char.id, c => {
      c.defense = oldDefense;
      c.dodge = num(value, c.dodge ?? 10);
      c.dodgeManual = true;
      c.dodgeLocked = true;
    });
    if (next) {
      const defense = $('defense');
      if (defense && document.activeElement !== defense) defense.value = num(next.defense, 10);
      try { if (typeof od42ScheduleCharacterSave === 'function') od42ScheduleCharacterSave(next); } catch (_) {}
    }
  }

  try { calculatedDodge = function od143CalculatedDodge(char = activeChar()){ return num(char?.dodge, 10); }; window.calculatedDodge = calculatedDodge; } catch (_) {}
  try { syncDodgeField = function od143SyncDodgeField(char = activeChar()){ forceManualDefenseDodge('syncDodgeField'); }; window.syncDodgeField = syncDodgeField; } catch (_) {}

  if (typeof updateDerivedStatsDisplay === 'function' && !updateDerivedStatsDisplay.__od143NoManualText) {
    const prev = updateDerivedStatsDisplay;
    updateDerivedStatsDisplay = function od143UpdateDerivedStatsDisplay(){
      const out = prev.apply(this, arguments);
      forceManualDefenseDodge('derived');
      return out;
    };
    updateDerivedStatsDisplay.__od143NoManualText = true;
    window.updateDerivedStatsDisplay = updateDerivedStatsDisplay;
  }

  document.addEventListener('beforeinput', event => {
    if (event.target?.matches?.('#defense,#dodge')) rememberManualValues();
  }, true);
  ...
  document.addEventListener('input', event => {
    ...
    if (field === 'defense') {
      lastDefenseInput = typed;
      const keepDodge = lastKnownDodge ?? activeChar()?.dodge ?? $('dodge')?.value ?? 10;
      saveDefenseOnly(typed);
      setTimeout(() => {
        const dodge = $('dodge');
        if (dodge && document.activeElement !== dodge) dodge.value = keepDodge;
        saveDefenseOnly(lastDefenseInput);
        forceManualDefenseDodge('defense-input');
      }, 0);
      setTimeout(() => forceManualDefenseDodge('defense-input-late'), 80);
    } else {
      lastDodgeInput = typed;
      const keepDefense = lastKnownDefense ?? activeChar()?.defense ?? $('defense')?.value ?? 10;
      saveDodgeOnly(typed);
      setTimeout(() => {
        const defense = $('defense');
        if (defense && document.activeElement !== defense) defense.value = keepDefense;
        saveDodgeOnly(lastDodgeInput);
        forceManualDefenseDodge('dodge-input');
      }, 0);
      setTimeout(() => forceManualDefenseDodge('dodge-input-late'), 80);
    }
    blankManualNotes();
  }, true);

  // 3 — abrir ficha pelo menu de fichas sempre abre como ficha da conta, não dentro da sessão.
  if (typeof initAccountCharacterEditor === 'function' && !initAccountCharacterEditor.__od143AccountOnly) {
    const oldInitAccount = initAccountCharacterEditor;
    initAccountCharacterEditor = function od143InitAccountCharacterEditor(charId = null){
      try { accountSheetMode = true; } catch (_) {}
      try { currentCampaignId = null; } catch (_) {}
      try { localStorage.removeItem(STORAGE.activeCampaign); } catch (_) {}
      const out = oldInitAccount.apply(this, arguments);
      try { accountSheetMode = true; currentCampaignId = null; localStorage.removeItem(STORAGE.activeCampaign); } catch (_) {}
      try { history.replaceState({ odRoute: 'v143-account-sheet' }, '', charId ? `/ficha/${encodeURIComponent(charId)}` : '/fichas'); } catch (_) {}
      return out;
    };
    initAccountCharacterEditor.__od143AccountOnly = true;
    window.initAccountCharacterEditor = initAccountCharacterEditor;
  }

  document.addEventListener('click', event => {
    const edit = event.target.closest('[data-edit-account-character], [data-open-account-character], [data-od71-open-character]');
    if (!edit) return;
    const id = edit.dataset.editAccountCharacter || edit.dataset.openAccountCharacter || edit.dataset.od71OpenCharacter;
    if (!id) return;
    event.preventDefault();
    event.stopPropagation();
    event.stopImmediatePropagation();
  ...
  // 4 e 5 — limpar vínculos órfãos de campanhas/sessões excluídas.
  function tableExists(id){ return campaigns().some(c => String(c.id) === String(id)); }
  function cleanupDeletedCampaignReferences(deletedId = null){
    const valid = new Set(campaigns().map(c => String(c.id)));
    const beforeMembers = members();
    const nextMembers = beforeMembers.filter(m => {
      const cid = String(m.campaignId || m.tableId || '');
      if (deletedId && cid === String(deletedId)) return false;
      return valid.has(cid);
    });
    if (nextMembers.length !== beforeMembers.length) saveMembers(nextMembers);

    const active = (() => { try { return localStorage.getItem(STORAGE.activeCampaign); } catch (_) { return null; } })();
    if ((deletedId && String(active) === String(deletedId)) || (active && !valid.has(String(active)))) {
      try { localStorage.removeItem(STORAGE.activeCampaign); } catch (_) {}
      try { currentCampaignId = null; } catch (_) {}
    }
    try { if (typeof renderCampaignMenu === 'function') renderCampaignMenu(); } catch (_) {}
  ...
  if (typeof deleteCampaign === 'function' && !deleteCampaign.__od143Unlink) {
    const oldDelete = deleteCampaign;
    deleteCampaign = function od143DeleteCampaign(campaignId){
      const out = oldDelete.apply(this, arguments);
      setTimeout(() => cleanupDeletedCampaignReferences(campaignId), 0);
      setTimeout(() => cleanupDeletedCampaignReferences(campaignId), 300);
      return out;
    };
    deleteCampaign.__od143Unlink = true;
    window.deleteCampaign = deleteCampaign;
  }

  if (typeof enterCampaign === 'function' && !enterCampaign.__od143GuardDeleted) {
    const oldEnter = enterCampaign;
    enterCampaign = async function od143EnterCampaign(campaignId){
      cleanupDeletedCampaignReferences();
      if (!tableExists(campaignId)) {
        try { localStorage.removeItem(STORAGE.activeCampaign); currentCampaignId = null; } catch (_) {}
        try { showSessions?.(); } catch (_) {}
        alert('Essa campanha não existe mais. O vínculo local foi removido.');
        return;
      }
      return oldEnter.apply(this, arguments);
    };
    enterCampaign.__od143GuardDeleted = true;
    window.enterCampaign = enterCampaign;
  }

  // reforça limpeza em eventos online de mesa excluída
  if (typeof od44EnsureSocket === 'function' && !od44EnsureSocket.__od143TableDeleteCleanup) {
    const oldEnsureSocket = od44EnsureSocket;
    od44EnsureSocket = function od143EnsureSocket(){
      const socket = oldEnsureSocket.apply(this, arguments);
      try {
        if (socket && !socket.__od143DeleteCleanup) {
          socket.__od143DeleteCleanup = true;
          socket.on('table:deleted', payload => {
            const id = payload?.tableId;
            if (id) {
              saveCampaigns(campaigns().filter(c => String(c.id) !== String(id)));
              cleanupDeletedCampaignReferences(id);
            }
          });
        }
      } catch (_) {}
    ...
    od44EnsureSocket.__od143TableDeleteCleanup = true;
    window.od44EnsureSocket = od44EnsureSocket;
  }

  // 6 — manter no máximo 5 backups por ficha, incluindo os antigos da v140/v141.
  function pruneBackups(){
    try {
      ...
      Object.keys(store || {}).forEach(id => {
        ...
        const clean = [];
        for (const item of list) {
          const sig = item?.signature || JSON.stringify(item?.character || {});
          if (seen.has(sig)) continue;
          seen.add(sig);
          clean.push(item);
        }
        if (clean.length !== list.length || clean.length > MAX_BACKUPS) changed = true;
        store[id] = clean.slice(0, MAX_BACKUPS);
      });
      if (changed) localStorage.setItem(BACKUP_KEY, JSON.stringify(store));
    } catch (_) {}
  }

  // 7 — barra rápida recolhida dentro do menu de três traços.
  function buildQuickbarMenuButton(){
    const panel = $('sessions-menu-panel');
    if (!panel || $('od143-quickbar-menu-btn')) return;
    const btn = document.createElement('button');
    btn.id = 'od143-quickbar-menu-btn';
    btn.type = 'button';
    btn.className = 'ghost-btn menu-entry';
    btn.textContent = 'Mostrar/Ocultar barra rápida';
    btn.addEventListener('click', event => {
      event.preventDefault();
      const hidden = localStorage.getItem('od143_quickbar_hidden') !== '0';
      localStorage.setItem('od143_quickbar_hidden', hidden ? '0' : '1');
      applyQuickbarVisibility();
    });
    const logout = $('sessions-logout');
    if (logout?.parentElement === panel) panel.insertBefore(btn, logout);
    else panel.appendChild(btn);
  }

  function applyQuickbarVisibility(){
    const bar = $('od140-quickbar');
    if (!bar) return;
    const show = localStorage.getItem('od143_quickbar_hidden') === '0';
    document.body.classList.toggle('od143-quickbar-enabled', show);
    bar.classList.toggle('od143-force-hidden', !show);
    if (!show) bar.classList.add('hidden');
    else if ($('app-screen')?.classList.contains('active')) bar.classList.remove('hidden');
    const btn = $('od143-quickbar-menu-btn');
    if (btn) btn.textContent = show ? 'Ocultar barra rápida' : 'Mostrar barra rápida';
  }

  // 8 — perfil da sessão abre configurações de conta, em vez de ser só decoração.
  function makeSessionProfileUseful(){
    document.querySelectorAll('.od90-user-menu-card, .od108-info-card, #main-topbar .brand').forEach(card => {
      if (!card || card.dataset.od143ProfileButton === '1') return;
      card.dataset.od143ProfileButton = '1';
      card.setAttribute('role', 'button');
      card.setAttribute('tabindex', '0');
      card.title = 'Abrir configurações da conta';
      card.addEventListener('click', event => {
        event.preventDefault();
        try {
          if (typeof od50OpenAccountSettings === 'function') od50OpenAccountSettings();
          else $('open-account-settings-btn')?.click();
        } catch (_) { $('open-account-settings-btn')?.click(); }
      });
  ...
  // servidor local: se o estado online trouxer menos mesas, remove órfãos locais.
  if (typeof od42RefreshTables === 'function' && !od42RefreshTables.__od143Cleanup) {
    const oldRefresh = od42RefreshTables;
    od42RefreshTables = async function od143RefreshTables(){
      const out = await oldRefresh.apply(this, arguments);
      cleanupDeletedCampaignReferences();
      return out;
    };
    od42RefreshTables.__od143Cleanup = true;
    window.od42RefreshTables = od42RefreshTables;
  }

  function boot(){
    pruneBackups();
    cleanupDeletedCampaignReferences();
    forceManualDefenseDodge('boot');
    buildQuickbarMenuButton();
    applyQuickbarVisibility();
    makeSessionProfileUseful();
  }

  if (document.readyState === 'loading') document.addEventListener('DOMContentLoaded', boot, { once: true }); else boot();
  [80, 300, 900, 1600].forEach(ms => setTimeout(boot, ms));

  const observer = new MutationObserver(() => {
    forceManualDefenseDodge('observer');
    buildQuickbarMenuButton();
    applyQuickbarVisibility();
    makeSessionProfileUseful();
  });
...
/* =========================
   V144 - hotfix Hostinger/carregamento e mesa online
   - API com timeout para não deixar a aba girando infinito.
   - Entrada em campanha online não depende de cache local antigo.
   - Boot não fica preso tentando abrir mesa apagada/inválida.
   - Defesa e Esquiva preservam o valor digitado como campos independentes.
========================= */
(function od144HostingerOnlineHotfix(){
  'use strict';
  if (window.__od144HostingerOnlineHotfixInstalled) return;
  window.__od144HostingerOnlineHotfixInstalled = true;
  window.ONE_DICE_CLIENT_VERSION = '1.44.0';

  const API_TIMEOUT_MS = 12000;
  const STORAGE_KEY_SESSION = 'od_online_session_v42';
  const $ = id => document.getElementById(id);

  function safeJson(value, fallback){ try { return JSON.parse(value); } catch (_) { return fallback; } }
  function safeGet(key, fallback){
    try { return typeof get === 'function' ? get(key, fallback) : safeJson(localStorage.getItem(key), fallback); }
    catch (_) { return fallback; }
  }
  ...
  function storeKey(name, fallback){ try { return STORAGE?.[name] || fallback; } catch (_) { return fallback; } }
  function localCharacters(){ return safeGet(storeKey('characters', 'od_characters'), []); }
  function localMembers(){
    try { return typeof getMembers === 'function' ? getMembers() : safeGet(storeKey('campaignMembers', 'od_campaign_members'), []); }
    catch (_) { return []; }
  }
  function localCampaigns(){
    try { return typeof getCampaigns === 'function' ? getCampaigns() : safeGet(storeKey('campaigns', 'od_campaigns'), []); }
    catch (_) { return []; }
  }
  function saveLocalMembers(items){
    try { if (typeof setMembers === 'function') setMembers(items); else safeSet(storeKey('campaignMembers', 'od_campaign_members'), items); }
    catch (_) {}
  }
  function saveLocalCampaigns(items){
    try { if (typeof setCampaigns === 'function') setCampaigns(items); else safeSet(storeKey('campaigns', 'od_campaigns'), items); }
    catch (_) {}
  }
  function token(){
    try {
      const raw = sessionStorage.getItem(STORAGE_KEY_SESSION) || localStorage.getItem(STORAGE_KEY_SESSION) || '';
      return safeJson(raw, {})?.token || '';
    } catch (_) { return ''; }
  }
  function normalizeUser(apiUser){
    if (typeof od42User === 'function') return od42User(apiUser);
    if (!apiUser) return null;
    return { id: apiUser.id, nick: apiUser.nick, realName: apiUser.real_name || apiUser.realName || apiUser.nick, name: apiUser.real_name || apiUser.realName || apiUser.nick };
  }
  function tableFromRow(row){
    if (typeof od42TableFromRow === 'function') return od42TableFromRow(row);
    return { id: row.id, name: row.name, code: row.invite_code || row.code, ownerId: row.owner_id || row.ownerId, settings: row.settings || {} };
  }
  function memberFromRow(row){
    if (typeof od42MemberFromRow === 'function') return od42MemberFromRow(row);
    const role = row.role === 'master' ? 'mestre' : row.role === 'master_player' ? 'mestre_jogador' : 'jogador';
    return { id: row.id, campaignId: row.table_id || row.campaignId || row.id, userId: row.user_id || row.userId, role, characterId: row.character_id || row.characterId || null };
  }
  function characterFromRow(row){
    if (typeof od42CharacterFromRow === 'function') return od42CharacterFromRow(row);
    const data = row?.data || row?.character_data || {};
    return { ...data, id: row?.id || row?.character_id || data.id, ownerId: row?.owner_id || row?.user_id || data.ownerId, name: row?.name || row?.character_name || data.name || 'Ficha' };
  }
  function mergeById(storageKey, items){
    try {
      if (typeof od42MergeById === 'function') return od42MergeById(storageKey, items);
      const old = safeGet(storageKey, []);
      const map = new Map(old.map(i => [String(i.id), i]));
      items.filter(Boolean).forEach(i => map.set(String(i.id), { ...(map.get(String(i.id)) || {}), ...i }));
      safeSet(storageKey, Array.from(map.values()));
    } catch (_) {}
  }
  async function api(path, options = {}){
    ...
    try {
      ...
      if (error?.name === 'AbortError') throw new Error('Servidor demorou para responder. Recarregue a página ou confira se o Node está rodando na Hostinger.');
      throw error;
    } finally {
  ...
  // Substitui a API base por uma versão com timeout. Isso impede carregamento eterno.
  try { window.od42Api = od42Api = api; } catch (_) { window.od42Api = api; }

  async function refreshCharactersSafe(){
    const data = await api('/api/characters');
    const owned = (data.characters || []).map(characterFromRow).filter(c => c?.id);
    const others = localCharacters().filter(c => c.ownerId !== currentUser?.id);
    safeSet(storeKey('characters', 'od_characters'), [...others, ...owned]);
    return owned;
  }

  async function refreshTablesSafe(){
    const data = await api('/api/tables');
    const tables = (data.tables || []).map(tableFromRow).filter(t => t?.id);
    const members = (data.tables || []).map(row => ({
      id: `${row.id}_${currentUser?.id}`,
      campaignId: row.id,
      userId: currentUser?.id,
      role: row.role === 'master' ? 'mestre' : row.role === 'master_player' ? 'mestre_jogador' : 'jogador',
      characterId: row.character_id || null
    }));
    saveLocalCampaigns(tables);
    saveLocalMembers(members);
    try { if (currentUser) mergeById(storeKey('users', 'od_users'), [currentUser]); } catch (_) {}
    return { tables, members };
  }

  async function loadTableStateSafe(tableId){
    const data = await api(`/api/tables/${encodeURIComponent(tableId)}/state`);
    const table = tableFromRow(data.table);
    const members = (data.members || []).map(memberFromRow);
    const users = (data.members || []).map(row => normalizeUser({ id: row.user_id, nick: row.nick, real_name: row.real_name, avatar_url: row.avatar_url })).filter(Boolean);
    const chars = (data.members || [])
      .filter(row => row.character_id && row.character_data)
      .map(row => characterFromRow({ id: row.character_id, owner_id: row.user_id, name: row.character_name, data: row.character_data }));

    const tables = localCampaigns().filter(c => String(c.id) !== String(table.id));
    saveLocalCampaigns([table, ...tables]);
    const allMembers = localMembers().filter(m => String(m.campaignId) !== String(table.id));
    saveLocalMembers([...allMembers, ...members]);
    mergeById(storeKey('users', 'od_users'), users);
    mergeById(storeKey('characters', 'od_characters'), chars);
    return { table, members, users, chars };
  }

  try { window.od42RefreshOwnCharacters = od42RefreshOwnCharacters = refreshCharactersSafe; } catch (_) { window.od42RefreshOwnCharacters = refreshCharactersSafe; }
  try { window.od42RefreshTables = od42RefreshTables = refreshTablesSafe; } catch (_) { window.od42RefreshTables = refreshTablesSafe; }
  try { window.od42LoadTableState = od42LoadTableState = loadTableStateSafe; } catch (_) { window.od42LoadTableState = loadTableStateSafe; }

  // Entrada em mesa: sempre consulta o servidor primeiro. Não bloqueia por cache local antigo.
  const previousEnter = typeof enterCampaign === 'function' ? enterCampaign : null;
  enterCampaign = async function od144EnterCampaign(campaignId){
    try {
      if (!campaignId) return typeof showSessions === 'function' ? showSessions() : null;
      await refreshCharactersSafe();
      await refreshTablesSafe();
      await loadTableStateSafe(campaignId);
      const member = localMembers().find(m => String(m.campaignId) === String(campaignId) && String(m.userId) === String(currentUser?.id));
      if (!member) {
        try { localStorage.removeItem(storeKey('activeCampaign', 'od_active_campaign')); } catch (_) {}
        if (typeof showSessions === 'function') showSessions();
        alert('Você não faz parte desta mesa ou ela foi removida.');
        return;
      }
      currentCampaignId = campaignId;
      safeSet(storeKey('activeCampaign', 'od_active_campaign'), campaignId);
      if (typeof initApp === 'function') initApp(campaignId);
    } catch (error) {
      console.warn('Falha ao abrir mesa:', error);
      try { localStorage.removeItem(storeKey('activeCampaign', 'od_active_campaign')); } catch (_) {}
      if (typeof showSessions === 'function') showSessions();
      alert(error?.message || 'Erro ao abrir mesa.');
    }
  ...
  // Boot seguro: se a mesa salva estiver apagada/travando, volta para a lista em vez de ficar carregando.
  od42Booted = false;
  od42Boot = async function od144Boot(){
    if (od42Booted) return;
    od42Booted = true;
    try { if (typeof od42WireForms === 'function') od42WireForms(); } catch (_) {}
    const raw = safeJson(sessionStorage.getItem(STORAGE_KEY_SESSION) || localStorage.getItem(STORAGE_KEY_SESSION) || 'null', null);
    if (!raw?.token) { if (typeof showAuth === 'function') showAuth(); return; }
    try {
      const data = await api('/api/auth/me');
      currentUser = normalizeUser(data.user);
      try { od42SetSession({ token: raw.token, user: currentUser }); } catch (_) {}
      try { setSessionValue(currentUser.id); } catch (_) {}
      mergeById(storeKey('users', 'od_users'), [currentUser]);
      await refreshCharactersSafe();
      const { members } = await refreshTablesSafe();
      const active = safeGet(storeKey('activeCampaign', 'od_active_campaign'), null);
      if (active && members.some(m => String(m.campaignId) === String(active) && String(m.userId) === String(currentUser.id))) {
        await enterCampaign(active);
      } else if (typeof showSessions === 'function') {
        showSessions();
      }
    } catch (error) {
      console.warn('Falha no boot online:', error);
      // Mantém login salvo quando for só timeout/rede; não apaga a sessão automaticamente.
      if (typeof showSessions === 'function' && token()) showSessions();
      else if (typeof showAuth === 'function') showAuth();
    }
  };
  window.od42Boot = od42Boot;

  // Defesa/Esquiva: último bloqueio contra camadas antigas que recalculam um campo pelo outro.
  let editingDefense = false;
  let editingDodge = false;
  function activeChar(){ try { return typeof currentChar === 'function' ? currentChar() : null; } catch (_) { return null; } }
  function mutateActive(fn){
    const char = activeChar();
    if (!char?.id) return null;
    const list = localCharacters();
    const idx = list.findIndex(c => String(c.id) === String(char.id));
    if (idx < 0) return null;
    fn(list[idx]);
    list[idx].updatedAt = Date.now();
    safeSet(storeKey('characters', 'od_characters'), list);
    return list[idx];
  }
  function pinManualFields(){
    const def = $('defense');
    const dod = $('dodge');
    if (def) { def.removeAttribute('readonly'); def.dataset.manualOnly = 'true'; }
    if (dod) { dod.removeAttribute('readonly'); dod.dataset.manualOnly = 'true'; }
  ...
  try { calculatedDodge = function od144CalculatedDodge(char = activeChar()){ return Number(char?.dodge ?? $('dodge')?.value ?? 10) || 10; }; window.calculatedDodge = calculatedDodge; } catch (_) {}
  try { syncDodgeField = function od144SyncDodgeField(){ pinManualFields(); }; window.syncDodgeField = syncDodgeField; } catch (_) {}

  document.addEventListener('input', event => {
    const target = event.target;
    if (!target?.matches?.('#defense,#dodge')) return;
    pinManualFields();
    if (target.id === 'defense') {
      editingDefense = true;
      const value = target.value;
      const dodgeValue = $('dodge')?.value;
      const changed = mutateActive(c => { c.defense = Number(value || 0); c.dodge = Number(dodgeValue || c.dodge || 0); c.dodgeManual = true; c.dodgeLocked = true; });
      setTimeout(() => { editingDefense = false; if ($('dodge') && document.activeElement?.id !== 'dodge') $('dodge').value = dodgeValue ?? changed?.dodge ?? ''; pinManualFields(); }, 0);
    }
    if (target.id === 'dodge') {
      editingDodge = true;
      const value = target.value;
      const defenseValue = $('defense')?.value;
      const changed = mutateActive(c => { c.defense = Number(defenseValue || c.defense || 0); c.dodge = Number(value || 0); c.dodgeManual = true; c.dodgeLocked = true; });
      setTimeout(() => { editingDodge = false; if ($('defense') && document.activeElement?.id !== 'defense') $('defense').value = defenseValue ?? changed?.defense ?? ''; pinManualFields(); }, 0);
    }
  }, true);

  if (typeof saveCurrentCharacter === 'function' && !saveCurrentCharacter.__od144ManualDefenseDodge) {
    const oldSave = saveCurrentCharacter;
    saveCurrentCharacter = function od144SaveCurrentCharacter(){
      const defValue = $('defense')?.value;
      const dodValue = $('dodge')?.value;
      const out = oldSave.apply(this, arguments);
      const changed = mutateActive(c => {
        if (defValue !== undefined) c.defense = Number(defValue || 0);
        if (dodValue !== undefined) c.dodge = Number(dodValue || 0);
        c.dodgeManual = true;
        c.dodgeLocked = true;
      });
      try { if (changed && typeof od42ScheduleCharacterSave === 'function') od42ScheduleCharacterSave(changed); } catch (_) {}
      if ($('defense') && document.activeElement?.id !== 'defense') $('defense').value = defValue ?? changed?.defense ?? '';
      if ($('dodge') && document.activeElement?.id !== 'dodge') $('dodge').value = dodValue ?? changed?.dodge ?? '';
      pinManualFields();
      return out;
    };
    saveCurrentCharacter.__od144ManualDefenseDodge = true;
    window.saveCurrentCharacter = saveCurrentCharacter;
  }

  window.addEventListener('error', event => {
    console.warn('Erro capturado pela camada v144:', event.error || event.message);
    setTimeout(() => {
      const active = document.querySelector('.screen.active');
      if (!active && typeof showSessions === 'function') showSessions();
    }, 100);
  });

  function boot(){
    pinManualFields();
    if (typeof od42WireForms === 'function') od42WireForms();
  }
  if (document.readyState === 'loading') document.addEventListener('DOMContentLoaded', boot, { once: true }); else boot();
  setTimeout(boot, 250);
  setTimeout(() => { try { od42Boot(); } catch (_) {} }, 350);
  setInterval(pinManualFields, 1500);
})();

/* =========================
   V145 - hotfix anti-carregamento infinito
   - Não entra automaticamente em mesa salva no navegador.
   - Socket.IO só conecta depois de entrar numa mesa.
   - Boot volta para a tela de mesas/fichas se qualquer chamada travar.
========================= */
(function od145StopInfiniteLoading(){
  'use strict';
  if (window.__od145StopInfiniteLoadingInstalled) return;
  window.__od145StopInfiniteLoadingInstalled = true;
  window.ONE_DICE_CLIENT_VERSION = '1.45.0';

  const SESSION_KEY = 'od_online_session_v42';
  const ACTIVE_KEY = (() => { try { return STORAGE?.activeCampaign || 'od_active_campaign'; } catch (_) { return 'od_active_campaign'; } })();

  function readSession(){
    try { return JSON.parse(sessionStorage.getItem(SESSION_KEY) || localStorage.getItem(SESSION_KEY) || 'null'); }
    catch (_) { return null; }
  }
  function clearActiveMesa(){
    try { localStorage.removeItem(ACTIVE_KEY); } catch (_) {}
    try { sessionStorage.removeItem(ACTIVE_KEY); } catch (_) {}
    try { if (typeof set === 'function') set(ACTIVE_KEY, null); } catch (_) {}
    try { currentCampaignId = null; } catch (_) {}
  }
  function showSafeHome(){
    clearActiveMesa();
    try {
      if (typeof showSessions === 'function') {
        showSessions();
        return;
      }
    } catch (_) {}
  ...
  async function withTimeout(promise, ms, label){
    ...
    const timeout = new Promise((_, reject) => {
      timer = setTimeout(() => reject(new Error(label || 'Tempo esgotado ao carregar.')), ms);
    });
    try { return await Promise.race([promise, timeout]); }
    finally { clearTimeout(timer); }
  }

  // Evita conexão global do Socket.IO enquanto o usuário ainda não entrou numa mesa.
  if (typeof od44EnsureSocket === 'function' && !od44EnsureSocket.__od145MesaOnly) {
    const previousEnsureSocket = od44EnsureSocket;
    od44EnsureSocket = function od145EnsureSocketOnlyInsideMesa(){
      try {
        if (!currentCampaignId) {
          if (od44Socket) {
            try { od44Socket.disconnect(); } catch (_) {}
      ...
      return previousEnsureSocket.apply(this, arguments);
    };
    od44EnsureSocket.__od145MesaOnly = true;
    window.od44EnsureSocket = od44EnsureSocket;
  }

  // Boot seguro: login permanece, mas não abre a última mesa automaticamente.
  od42Booted = false;
  od42Boot = async function od145Boot(){
    if (od42Booted) return;
    od42Booted = true;
    try { if (typeof od42WireForms === 'function') od42WireForms(); } catch (_) {}
    ...
    try {
      const me = await withTimeout(od42Api('/api/auth/me'), 10000, 'Servidor demorou para confirmar o login.');
      currentUser = typeof od42User === 'function' ? od42User(me.user) : me.user;
      try { od42SetSession({ token: session.token, user: currentUser }); } catch (_) {}
      try { setSessionValue(currentUser.id); } catch (_) {}
      try { if (typeof od42MergeById === 'function') od42MergeById(STORAGE.users, [currentUser]); } catch (_) {}
      try { await withTimeout(od42RefreshOwnCharacters(), 10000, 'Servidor demorou para carregar fichas.'); } catch (error) { console.warn(error); }
      try { await withTimeout(od42RefreshTables(), 10000, 'Servidor demorou para carregar mesas.'); } catch (error) { console.warn(error); }
      showSafeHome();
    } catch (error) {
      console.warn('Boot seguro v145 falhou:', error);
      showSafeHome();
    }
  };
  window.od42Boot = od42Boot;

  // Reentrada manual em mesa com proteção. Só ocorre quando o usuário clica em Entrar.
  if (typeof enterCampaign === 'function' && !enterCampaign.__od145ManualOnly) {
    const previousEnterCampaign = enterCampaign;
    enterCampaign = async function od145EnterCampaignManual(campaignId){
      try {
        if (!campaignId) return showSafeHome();
        await withTimeout(previousEnterCampaign.call(this, campaignId), 15000, 'Mesa demorou para carregar.');
      } catch (error) {
        console.warn('Falha ao entrar na mesa v145:', error);
        clearActiveMesa();
        showSafeHome();
        alert(error?.message || 'Erro ao abrir mesa.');
      }
    };
    enterCampaign.__od145ManualOnly = true;
    window.enterCampaign = enterCampaign;
  }

  // Última segurança: quando a janela terminar de carregar, se nenhuma tela estiver ativa, volta para home segura.
  window.addEventListener('load', () => {
    setTimeout(() => {
      try {
        const active = document.querySelector('.screen.active');
        if (!active) showSafeHome();
      } catch (_) {}
    }, 800);
  }, { once: true });

  // Executa depois de todas as camadas antigas agendadas.
  setTimeout(() => { try { od42Boot(); } catch (error) { console.warn(error); showSafeHome(); } }, 900);
})();


/* =========================
   V146 - hotfix de boot travado e mesa fantasma
   - Bloqueia initApp sem mesa válida.
   - Remove fallback automático para mesa antiga salva no localStorage.
   - Força tela de mesas/fichas quando o app abre preso na ficha vazia.
   - Adiciona botão de recuperação no menu de três traços.
========================= */
(function od146BootRecovery(){
  'use strict';
  if (window.__od146BootRecoveryInstalled) return;
  window.__od146BootRecoveryInstalled = true;
  window.ONE_DICE_CLIENT_VERSION = '1.46.0';

  const $ = id => document.getElementById(id);
  const ACTIVE_KEYS = ['od_active_campaign'];
  try { if (typeof STORAGE?.activeCampaign === 'string') ACTIVE_KEYS.push(STORAGE.activeCampaign); } catch (_) {}

  function removeActiveCampaign(){
    for (const key of [...new Set(ACTIVE_KEYS)]) {
      try { localStorage.removeItem(key); } catch (_) {}
      try { sessionStorage.removeItem(key); } catch (_) {}
    }
    try { currentCampaignId = null; } catch (_) {}
    try { accountSheetMode = false; } catch (_) {}
    try { document.body.classList.remove('master-dashboard-mode','master-sheet-open','readonly-character','route-mobile-focus','route-chat-focus','route-dados-focus','route-drops-focus','route-fichas-focus','route-ficha-focus'); } catch (_) {}
  }

  function readSession(){
    try {
      const raw = sessionStorage.getItem('od_online_session_v42') || localStorage.getItem('od_online_session_v42') || 'null';
      return JSON.parse(raw);
    } catch (_) { return null; }
  }

  function localTables(){
    try { return typeof getCampaigns === 'function' ? getCampaigns() : JSON.parse(localStorage.getItem('od_campaigns') || '[]'); }
    catch (_) { return []; }
  }
  function localMembers(){
    try { return typeof getMembers === 'function' ? getMembers() : JSON.parse(localStorage.getItem('od_campaign_members') || '[]'); }
    catch (_) { return []; }
  }
  function tableIsValidForUser(tableId){
    if (!tableId) return false;
    const uid = String(currentUser?.id || readSession()?.user?.id || '');
    if (!uid) return false;
    const tables = localTables();
    const exists = tables.some(t => String(t.id) === String(tableId));
    if (!exists) return false;
    return localMembers().some(m => String(m.campaignId) === String(tableId) && String(m.userId) === uid);
  }
  ...
  function forceScreen(name){
    ...
    [auth, sessions, app].forEach(el => el && el.classList.remove('active'));
    if (overlay) overlay.classList.remove('active');
    if (name === 'auth' && auth) auth.classList.add('active');
    else if (name === 'app' && app) app.classList.add('active');
    else if (sessions) sessions.classList.add('active');
  }

  function safeHome(reason){
    console.warn('[One Dice V146] Recuperando tela inicial:', reason || 'sem motivo informado');
    removeActiveCampaign();
    const session = readSession();
    if (!session?.token && !currentUser) {
      try { if (typeof showAuth === 'function') showAuth(); else forceScreen('auth'); } catch (_) { forceScreen('auth'); }
      return;
    }
    try { currentUser = currentUser || session.user || currentUser; } catch (_) {}
    try {
      if (typeof showSessions === 'function') showSessions();
      else forceScreen('sessions');
    } catch (_) { forceScreen('sessions'); }
    try { renderAccountCharacterMenu?.(); } catch (_) {}
    try { renderCampaignMenu?.(); } catch (_) {}
    try { history.replaceState({ odRoute: 'v146', path: '/mesas' }, '', '/mesas'); } catch (_) {}
  }

  // initApp antigo fazia: currentCampaignId = campaignId || localStorage.activeCampaign.
  // Isso reabria mesa fantasma e prendia o usuário na ficha vazia. Agora só entra com mesa válida.
  if (typeof initApp === 'function' && !initApp.__od146Guarded) {
    const previousInitApp = initApp;
    initApp = function od146InitAppGuard(campaignId){
      const explicit = campaignId || currentCampaignId || null;
      if (!explicit || !tableIsValidForUser(explicit)) {
        safeHome('initApp sem mesa válida');
        return null;
      }
      currentCampaignId = explicit;
      return previousInitApp.apply(this, [explicit]);
    };
    initApp.__od146Guarded = true;
    window.initApp = initApp;
  }

  if (typeof enterCampaign === 'function' && !enterCampaign.__od146Guarded) {
    const previousEnter = enterCampaign;
    enterCampaign = async function od146EnterCampaignGuard(campaignId){
      if (!campaignId) return safeHome('enterCampaign sem id');
      // Se ainda não chegou a lista do servidor, tenta atualizar uma vez antes de rejeitar.
      if (!tableIsValidForUser(campaignId)) {
        try { await od42RefreshTables?.(); } catch (_) {}
      }
      if (!tableIsValidForUser(campaignId)) return safeHome('mesa inexistente ou usuário sem vínculo');
      currentCampaignId = campaignId;
      try { localStorage.setItem('od_active_campaign', String(campaignId)); } catch (_) {}
      return previousEnter.apply(this, [campaignId]);
    };
    enterCampaign.__od146Guarded = true;
    window.enterCampaign = enterCampaign;
  }

  // Boot definitivo: não interpreta /mesa antiga nem localStorage antigo. Primeiro mostra o hub.
  try { od42Booted = false; } catch (_) {}
  od42Boot = async function od146Boot(){
    if (window.__od146BootDone) return;
    window.__od146BootDone = true;
    removeActiveCampaign();
    const session = readSession();
    if (!session?.token) return safeHome('sem sessão');
    try {
      if (typeof od42Api === 'function') {
        const me = await Promise.race([
          od42Api('/api/auth/me'),
          new Promise((_, reject) => setTimeout(() => reject(new Error('timeout /api/auth/me')), 8000))
        ]);
        currentUser = typeof od42User === 'function' ? od42User(me.user) : me.user;
        try { od42SetSession?.({ token: session.token, user: currentUser }); } catch (_) {}
      } else {
      ...
      try { await Promise.race([od42RefreshOwnCharacters?.(), new Promise(resolve => setTimeout(resolve, 8000))]); } catch (_) {}
      try { await Promise.race([od42RefreshTables?.(), new Promise(resolve => setTimeout(resolve, 8000))]); } catch (_) {}
      safeHome('boot concluído');
    } catch (error) {
      console.warn('[One Dice V146] Boot online falhou:', error);
      safeHome('falha no boot online');
    }
  };
  window.od42Boot = od42Boot;

  // Se qualquer camada antiga ativar a tela app sem mesa válida, volta para o hub.
  function watchdog(){
    const appActive = $('app-screen')?.classList.contains('active');
    if (!appActive) return;
    const valid = tableIsValidForUser(currentCampaignId);
    const rootOrHub = location.pathname === '/' || location.pathname === '/mesas' || location.pathname === '/campanhas';
    if (rootOrHub || !valid) safeHome('watchdog encontrou app sem mesa válida');
  }

  let checks = 0;
  const interval = setInterval(() => {
    checks += 1;
    try { watchdog(); } catch (error) { console.warn(error); }
    if (checks > 40) clearInterval(interval);
  }, 250);
  window.addEventListener('load', () => setTimeout(watchdog, 700), { once: true });
  setTimeout(() => { try { od42Boot(); } catch (_) { safeHome('erro ao executar boot v146'); } }, 1200);

  // Botão de emergência no menu de 3 traços para limpar só o estado de navegação, sem apagar fichas/conta.
  function installRecoveryButton(){
    const menus = [document.querySelector('.floating-menu'), document.querySelector('#hamburger-menu'), document.querySelector('.hamburger-menu'), document.querySelector('.quick-actions-menu')].filter(Boolean);
    const host = menus[0] || document.body;
    if (document.getElementById('od146-recover-navigation')) return;
    const btn = document.createElement('button');
    btn.type = 'button';
    btn.id = 'od146-recover-navigation';
    btn.textContent = 'Corrigir tela travada';
    btn.title = 'Limpa a mesa antiga salva e volta para Fichas/Mesas';
    btn.style.cssText = 'position:fixed;right:18px;bottom:18px;z-index:99999;padding:10px 12px;border:2px solid #d22;background:#111;color:#fff;border-radius:10px;font-weight:700;box-shadow:0 0 16px rgba(0,0,0,.4);';
    btn.addEventListener('click', () => safeHome('botão de recuperação'));
    host.appendChild(btn);
    setTimeout(() => { if (btn && !btn.matches(':hover')) btn.style.opacity = '0.35'; }, 5000);
    btn.addEventListener('mouseenter', () => btn.style.opacity = '1');
    btn.addEventListener('mouseleave', () => btn.style.opacity = '0.35');
  }
  if (document.readyState === 'loading') document.addEventListener('DOMContentLoaded', installRecoveryButton, { once: true });
  else installRecoveryButton();
})();
