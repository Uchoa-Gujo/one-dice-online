...
(function od139EarlyGuards(){
  'use strict';
  window.ONE_DICE_CLIENT_VERSION = '1.53.0';
...
(function od152StableLayoutFixes(){
  ...
  function setupQuickbar(){
    ...
    if (bar && localStorage.getItem('od152_quickbar_visible') !== '1') bar.classList.add('hidden');
    const panel = $('sessions-menu-panel');
    if (panel && !$('od152-toggle-quickbar')) {
      const btn = document.createElement('button');
      btn.id = 'od152-toggle-quickbar';
      btn.type = 'button';
      btn.className = 'ghost-btn menu-entry';
      btn.textContent = bar && !bar.classList.contains('hidden') ? 'Ocultar barra de ações' : 'Mostrar barra de ações';
      btn.addEventListener('click', () => {
        const targetBar = $('od140-quickbar');
        if (!targetBar) return;
        const willShow = targetBar.classList.contains('hidden');
        targetBar.classList.toggle('hidden', !willShow);
        localStorage.setItem('od152_quickbar_visible', willShow ? '1' : '0');
...
/* =========================
   V153 - correção cirúrgica sem trocar layout
   - Neutraliza funções legadas locais de Esquiva que ainda gravavam fórmula.
   - Defesa e Esquiva são salvas como campos manuais independentes.
   - Barra de ações fica realmente recolhida por padrão e só aparece pelo menu.
========================= */
(function od153ManualDefenseDodgeAndQuickbar(){
  'use strict';
  if (window.__od153ManualDefenseDodgeAndQuickbar) return;
  window.__od153ManualDefenseDodgeAndQuickbar = true;
  window.ONE_DICE_CLIENT_VERSION = '1.53.0';
  ...
  const charsKey = () => {
    try { return STORAGE.characters; } catch (_) { return 'od_characters'; }
  };
  const activeId = () => {
    try { return currentCharacterId || null; } catch (_) { return window.currentCharacterId || null; }
  };
  const readChars = () => {
    try { return typeof get === 'function' ? get(charsKey(), []) : JSON.parse(localStorage.getItem(charsKey()) || '[]'); }
    catch (_) { return []; }
  };
  const writeChars = list => {
    try { if (typeof set === 'function') set(charsKey(), list); else localStorage.setItem(charsKey(), JSON.stringify(list)); }
    catch (_) {}
  };
  const activeChar = () => {
    const id = activeId();
    return readChars().find(c => String(c.id) === String(id)) || null;
  };
  const writeManualPair = (changedId, rawValue) => {
    const id = activeId();
    if (!id) return null;
    const list = readChars();
    const idx = list.findIndex(c => String(c.id) === String(id));
    if (idx < 0) return null;
    const c = { ...list[idx] };
    const previousDefense = toNum(c.defense ?? c.defesa, 10);
    const previousDodge = toNum(c.dodge, 10);
    if (changedId === 'defense') c.defense = toNum(rawValue, previousDefense);
    if (changedId === 'dodge') c.dodge = toNum(rawValue, previousDodge);
    c.dodgeManual = true;
    c.dodgeLocked = true;
    c.updatedAt = Date.now();
    list[idx] = c;
  ...
  const paintPair = (char = activeChar(), keepActive = true) => {
    if (!char) return;
    const defense = $('defense');
    const dodge = $('dodge');
    if (defense) {
      defense.removeAttribute('readonly');
      defense.dataset.manualOnly = 'true';
      if (!keepActive || document.activeElement !== defense) defense.value = toNum(char.defense ?? char.defesa, 10);
    }
    if (dodge) {
      dodge.removeAttribute('readonly');
      dodge.dataset.manualOnly = 'true';
      dodge.title = 'Campo manual independente';
      if (!keepActive || document.activeElement !== dodge) dodge.value = toNum(char.dodge, 10);
    }
    hideNotes();
  };

  try { calculatedDodge = function od153CalculatedDodge(char = activeChar()){ return toNum(char?.dodge, 10); }; window.calculatedDodge = calculatedDodge; } catch (_) {}
  try { syncDodgeField = function od153SyncDodgeField(char = activeChar()){ paintPair(char); }; window.syncDodgeField = syncDodgeField; } catch (_) {}
  window.od153PaintDefenseDodge = paintPair;

  // Depois de qualquer handler legado, restaura o par salvo manualmente.
  function onManualField(event){
    const target = event.target;
    if (!target || (target.id !== 'defense' && target.id !== 'dodge')) return;
    const char = writeManualPair(target.id, target.value);
    hideNotes();
    setTimeout(() => paintPair(char || activeChar(), true), 0);
    setTimeout(() => paintPair(activeChar(), true), 30);
    setTimeout(() => paintPair(activeChar(), true), 120);
    try { if (typeof od42ScheduleCharacterSave === 'function') od42ScheduleCharacterSave(char || activeChar()); } catch (_) {}
  }
  document.addEventListener('input', onManualField, true);
  document.addEventListener('change', onManualField, true);

  if (typeof saveCurrentCharacter === 'function' && !saveCurrentCharacter.__od153ManualPair) {
    const previousSave = saveCurrentCharacter;
    saveCurrentCharacter = function od153SaveCurrentCharacter(...args){
      const beforeDefense = $('defense')?.value;
      const beforeDodge = $('dodge')?.value;
      const result = previousSave.apply(this, args);
      if ($('defense') || $('dodge')) {
        const list = readChars();
        const id = activeId();
        ...
        if (idx >= 0) {
          list[idx] = {
            ...list[idx],
            defense: toNum(beforeDefense, toNum(list[idx].defense ?? list[idx].defesa, 10)),
            dodge: toNum(beforeDodge, toNum(list[idx].dodge, 10)),
            dodgeManual: true,
            dodgeLocked: true,
            updatedAt: Date.now()
          };
          writeChars(list);
          paintPair(list[idx], true);
        }
      }
      return result;
    };
    saveCurrentCharacter.__od153ManualPair = true;
    window.saveCurrentCharacter = saveCurrentCharacter;
  }

  const setQuickbarVisible = visible => {
    document.body.classList.toggle('od153-quickbar-visible', !!visible);
    try { localStorage.setItem('od153_quickbar_visible', visible ? '1' : '0'); } catch (_) {}
    const bar = $('od140-quickbar');
    if (bar) bar.classList.toggle('hidden', !visible);
    const btn = $('od153-toggle-quickbar');
    if (btn) btn.textContent = visible ? 'Ocultar barra de ações' : 'Mostrar barra de ações';
  };
  function setupQuickbarToggle(){
    const visible = localStorage.getItem('od153_quickbar_visible') === '1';
    setQuickbarVisible(visible);
    const panel = $('sessions-menu-panel') || document.querySelector('.sessions-menu-panel');
    if (panel && !$('od153-toggle-quickbar')) {
      const btn = document.createElement('button');
      btn.id = 'od153-toggle-quickbar';
      btn.type = 'button';
      btn.className = 'ghost-btn menu-entry';
      btn.textContent = visible ? 'Ocultar barra de ações' : 'Mostrar barra de ações';
      btn.addEventListener('click', () => setQuickbarVisible(!document.body.classList.contains('od153-quickbar-visible')));
      const logout = $('sessions-logout');
      if (logout && logout.parentNode === panel) panel.insertBefore(btn, logout);
  ...
  function boot(){
    hideNotes();
    paintPair(activeChar());
    setupQuickbarToggle();
  }
  if (document.readyState === 'loading') document.addEventListener('DOMContentLoaded', boot, { once:true }); else boot();
  [50, 250, 800, 1600, 3000].forEach(ms => setTimeout(boot, ms));
  try { new MutationObserver(() => { hideNotes(); setupQuickbarToggle(); }).observe(document.body, { childList:true, subtree:true }); } catch (_) {}
})();
