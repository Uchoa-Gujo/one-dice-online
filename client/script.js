...
(function od67to69Routes(){
  ...
  document.addEventListener('click', event => {
    ...
    if (tabBtn) {
      // V1815: aba interna da ficha não deve navegar para /inicio nem recalcular rota.
      return;
    }
    const mobileTab = event.target.closest('[data-mobile-tab]');
...
/* =========================
   ...
   - Imagem de capa por URL ou arquivo convertido em Data URL
...
(function od86CampaignDashboardPatch(){
    ...
    dialog.innerHTML = `
      <form method="dialog" class="modal-card od86-editor-card">
        ...
        <div class="od86-editor-grid">
          ...
          <label>Ou enviar capa
            <input id="od86-campaign-logo-file" type="file" accept="image/*" />
          </label>
          <div class="od86-logo-preview-box">
            <span>Capa</span>
            <img id="od86-campaign-logo-preview" src="assets/logo.jpg" alt="Prévia da capa" />
          </div>
        </div>
  ...
  function od86RenderCampaigns(force = false) {
    ...
    const content = document.getElementById('od71-content');
    if (!content) return;
    if (!force && content.classList.contains('od190-campaigns-ready')) return;
    rendering = true;
    try {
      const campaigns = typeof getCampaigns === 'function' ? getCampaigns() : [];
      const allMembers = typeof getMembers === 'function' ? getMembers() : [];
      const members = allMembers.filter(m => String(m.userId) === String(currentUser?.id));
      const userCampaigns = members
        .map(member => ({ member, campaign: campaigns.find(c => String(c.id) === String(member.campaignId)) }))
        .filter(x => x.campaign)
        .slice(0, LIMIT);

      content.classList.remove('od86-campaigns-ready');
      content.classList.add('od86-campaigns-ready', 'od190-campaigns-ready');
      content.innerHTML = `
        <section class="od190-campaign-hero">
          <div class="od190-ornament"><span></span><i></i><span></span></div>
          <div class="od190-campaign-hero-grid">
            <div class="od190-campaign-copy">
              <p class="od190-eyebrow">Biblioteca de aventuras</p>
              <h1>Suas Campanhas</h1>
              <p class="od190-subtitle">Escolha uma porta, continue sua lenda e mantenha cada mesa organizada para jogadores e mestres.</p>
              <div class="od190-count"><strong>${userCampaigns.length}</strong>/<span>${LIMIT}</span> campanhas vinculadas</div>
            </div>
            <div class="od190-campaign-actions-top">
              <button class="od190-btn od190-btn-ghost" type="button" id="od86-open-join">↪ Entrar por Código</button>
              <button class="od190-btn od190-btn-primary" type="button" id="od86-new-campaign">+ Nova Campanha</button>
            </div>
          </div>
        </section>
        <div class="od190-join-form" id="od86-join-form">
          <input id="od86-join-code" maxlength="5" placeholder="Código da campanha" />
          <button class="od190-btn od190-btn-primary" type="button" id="od86-join-confirm">Entrar</button>
        </div>
        <section class="od190-campaign-list" id="od86-campaign-list"></section>`;

      const list = document.getElementById('od86-campaign-list');
      if (!userCampaigns.length) {
        list.innerHTML = `<div class="od190-empty">
          <strong>Nenhuma campanha encontrada.</strong>
          <span>Crie uma nova campanha ou entre com um código para começar sua próxima história.</span>
        </div>`;
        od86SyncNav();
        return;
      }
      ...
      const chars = get(STORAGE.characters, []);
      list.innerHTML = userCampaigns.map(({ member, campaign }, index) => {
        const char = chars.find(c => String(c.id) === String(member.characterId));
        const isOwner = String(campaign.ownerId) === String(currentUser?.id) || ['mestre','master','mestre_jogador','master_player'].includes(String(member.role || '').toLowerCase());
        const playerCount = od86CampaignCount(campaign.id, campaign);
        const description = od86CampaignDescription(campaign) || 'Uma história pronta para ser escrita. O mestre ainda não definiu o chamado desta campanha.';
        const cover = od86CampaignLogo(campaign);
        const code = campaign.code || '-----';
        const roleLabel = od86RoleLabel(member.role);
        const system = campaign.systemType || campaign.systemModel || campaign.model || 'D20';
        const linked = char ? `${escapeHtml(char.name || 'Personagem')}<small>${escapeHtml(char.race || 'Raça')} • Nv. ${escapeHtml(char.level || 1)} • ${escapeHtml(char.className || 'Classe')}</small>` : `Sem ficha vinculada<small>Escolha uma ficha antes de jogar.</small>`;
        return `
          <article class="od190-campaign-card ${isOwner ? 'od190-master-card' : 'od190-player-card'}" style="--od190-cover:url('${escapeHtml(cover)}')">
            <div class="od190-campaign-cover">
              <img src="${escapeHtml(cover)}" alt="" />
              <div class="od190-banner"><span>${escapeHtml(isOwner ? 'Mestre' : 'Jogador')}</span></div>
            </div>
            <div class="od190-campaign-body">
              <div class="od190-card-head">
                <div>
                  <span class="od190-card-index">Campanha ${index + 1}</span>
                  <h2>${escapeHtml(campaign.name || 'Campanha')}</h2>
                </div>
                <div class="od190-role-seal">${escapeHtml(roleLabel)}</div>
              </div>
              <p class="od190-campaign-lore">${escapeHtml(description)}</p>
              <div class="od190-tags">
                <span>⚙ ${escapeHtml(system)}</span>
                <span>👥 ${escapeHtml(playerCount)} jogador${playerCount === 1 ? '' : 'es'}</span>
                <span>🔑 ${escapeHtml(code)}</span>
              </div>
              <div class="od190-linked">
                <span>Personagem vinculado</span>
                <div>${linked}</div>
              </div>
            </div>
            <div class="od190-campaign-actions">
              <button class="od190-card-btn primary" type="button" data-enter-campaign="${escapeHtml(campaign.id)}">Acessar</button>
              <button class="od190-card-btn" type="button" data-choose-campaign-char="${escapeHtml(campaign.id)}">Escolher Ficha</button>
              ${isOwner ? `<button class="od190-card-btn" type="button" data-od86-edit-campaign="${escapeHtml(campaign.id)}">Editar Portal</button><button class="od190-card-btn" type="button" data-copy-code="${escapeHtml(code)}">Copiar Código</button><button class="od190-card-btn danger" type="button" data-delete-campaign="${escapeHtml(campaign.id)}">Excluir</button>` : `<button class="od190-card-btn danger" type="button" data-leave-campaign="${escapeHtml(campaign.id)}">Sair</button>`}
            </div>
          </article>`;
      }).join('');
      od86SyncNav();
...
/* =========================
   V180.10 - Botão de fichas refeito sem perfil de sessão
   Remove o antigo V90: sem card de perfil, sem imagem que some.
========================= */
(function od1810SheetButtonFinal(){
  'use strict';
  if (window.__od1810SheetButtonFinalInstalled) return;
  window.__od1810SheetButtonFinalInstalled = true;

  function $(id){ return document.getElementById(id); }

  function cleanSessionProfile(){
    document.querySelectorAll('.od90-profile-card,[data-od90-profile-card],.session-profile-card,.account-profile-card,.menu-profile-card,#session-profile-card,#account-menu-profile').forEach(el => el.remove());
  }

  function buildButton(btn){
    if (!btn) return;
    btn.classList.add('od1810-sheet-btn-final');
    btn.classList.remove('od90-sheet-stable-btn');
    btn.title = 'Minhas Fichas';
    btn.setAttribute('aria-label', 'Minhas Fichas');
    btn.innerHTML = '<span class="od1810-sheet-icon" aria-hidden="true"><span></span></span>';
  }

  function run(){
    cleanSessionProfile();
    buildButton($('toggle-account-panel-btn'));
    buildButton($('sidebar-dock-btn'));
    document.querySelectorAll('.sessions-sheet-toggle').forEach(buildButton);
  }

  document.addEventListener('click', event => {
    if (event.target.closest?.('#sessions-menu-btn,.topbar-menu-toggle,#toggle-account-panel-btn,#sidebar-dock-btn')) {
      setTimeout(run, 0);
      setTimeout(run, 120);
    }
  }, true);
  ...
  window.od1810SheetButtonFinal = { run, cleanSessionProfile };
})();
...
(function od108UsefulInfoMenu(){
  ...
  function renderMenuPanel(){
    document.querySelectorAll('#sessions-menu-panel .od90-user-menu-card, #sessions-menu-panel .od75-account-menu-card, .od90-user-menu-card, .od75-account-menu-card, .od108-panel-user-info').forEach(el => el.remove());
  }
...
(function od164StableCleanRoutes(){
  ...
  function install(){
    ...
    document.addEventListener('click', event => {
      if (event.target.closest?.('[data-od71-tab], [data-od75-tab], [data-edit-account-character], [data-open-account-character], [data-enter-campaign], #back-to-sessions-btn, #campaign-character-btn')) syncRoute(180);
    }, true);
...
(function od1715ScrollAndSmartCollapse(){
  ...
  function renderAttackSummary(char = current()){
    ...
    const html = attacks.map((atk, index) => `<button type="button" class="od1715-card-mini od1715-attack-mini od1817-attack-damage-roll" data-od1817-attack-index="${esc(index)}" title="Rolar dano de ${esc(atk.name || 'Ataque')}">
...
/* =========================
   V177.5 - Hotfix de reload/atualização mantendo rota e sessão
   Problema:
   - Ao dar F5 em /personagem/xxx ou /inicio, a tela podia cair no login
     ou voltar para a escolha de mesa antes da sessão online terminar de carregar.
   Correção:
   - Guarda a rota original.
   - Espera a sessão/personagens/mesas carregarem.
   - Restaura a ficha ou a tela correta depois do boot online.
========================= */
(function od1775ReloadRouteRestore(){
  'use strict';
  ...
  const IS_DEEP_ROUTE = !!INITIAL_PARTS.length && INITIAL_PARTS[0] !== 'login';
  let restoring = false;
  let restored = false;
  ...
  if (IS_DEEP_ROUTE && hasSessionToken()) {
    restoring = true;
    document.documentElement.classList.add('od1775-restoring-route');
    document.body?.classList?.add('od1775-restoring-route');
    try { sessionStorage.setItem('od_pending_route', INITIAL_PATH); } catch (_) {}
  }
  ...
  function ownCharacters(){
    try {
      const all = typeof get === 'function' && typeof STORAGE !== 'undefined' ? get(STORAGE.characters, []) : [];
      return all.filter(c => !currentUser?.id || String(c.ownerId || '') === String(currentUser.id));
    } catch (_) {
      return [];
    }
  }

  function resolveCharacter(token){
    const chars = ownCharacters();
    const raw = String(token || '').trim();
    if (!raw) return chars[0] || null;
    const wanted = slugify(raw);
    return chars.find(c => String(c.id) === raw)
      || chars.find(c => slugify(c.name) === wanted)
      || chars.find(c => slugify(c.name).includes(wanted))
      || null;
  }
  ...
  function setHubTab(tab){
    ...
    setTimeout(() => {
      try {
        const btn = document.querySelector(`[data-od71-tab="${tab}"], [data-od75-tab="${tab}"]`);
        if (btn) btn.click();
        const path = tab === 'characters' ? '/personagens' : tab === 'campaigns' ? '/campanhas' : '/inicio';
        history.replaceState({ od1775Restored: true, tab }, '', path);
      } catch (_) {}
    }, 80);
  }

  async function restoreRoute(){
    if (restored || !currentUser) return false;
    restored = true;

    const parts = INITIAL_PARTS;
    const query = new URLSearchParams(location.search || '');

    try {
      if (!parts.length || parts[0] === 'inicio') {
        setHubTab('home');
        return true;
      }

      if (parts[0] === 'personagens' || parts[0] === 'fichas') {
        setHubTab(parts[0] === 'personagens' ? 'characters' : 'characters');
        return true;
      }

      if (parts[0] === 'campanhas' || parts[0] === 'mesas') {
        setHubTab('campaigns');
        return true;
      }

      // Compatibilidade com URL antiga/amigável: /personagem/nome-da-ficha
      if (parts[0] === 'personagem' || parts[0] === 'ficha') {
        const char = resolveCharacter(parts[1] || currentCharacterId);
        if (char && typeof initAccountCharacterEditor === 'function') {
          initAccountCharacterEditor(char.id);
          activateTab(query.get('tab') || 'resumo');
          return true;
        }
        setHubTab('characters');
        return true;
      }

      // Mesa: mantém comportamento antigo, mas garante recarregamento do estado online.
      if (parts[0] === 'mesa') {
        const tableId = parts[1];
        if (tableId && typeof enterCampaign === 'function') {
          try {
            const memberOk = typeof getMembers === 'function'
              ? getMembers().some(m => String(m.campaignId) === String(tableId) && String(m.userId) === String(currentUser.id))
              : true;
            if (memberOk) {
              await enterCampaign(tableId);
              if ((parts[2] === 'ficha' || parts[2] === 'personagem') && parts[3]) {
                const charId = parts[3];
                const exists = typeof get === 'function' && typeof STORAGE !== 'undefined'
                  ? get(STORAGE.characters, []).some(c => String(c.id) === String(charId) || slugify(c.name) === slugify(charId))
                  : false;
                if (exists && typeof loadCharacter === 'function') {
                  const found = get(STORAGE.characters, []).find(c => String(c.id) === String(charId) || slugify(c.name) === slugify(charId));
                  loadCharacter(found?.id || charId);
                }
                activateTab(query.get('tab') || 'resumo');
              }
              return true;
      ...
      try { sessionStorage.removeItem('od_pending_route'); } catch (_) {}
    }

    return false;
  }

  // Evita que uma rota profunda com sessão válida seja substituída por /login durante o boot.
  if (typeof showAuth === 'function' && !showAuth.__od1775RouteRestore) {
    const previousShowAuth = showAuth;
    showAuth = function od1775ShowAuth(){
      if (restoring && hasSessionToken() && IS_DEEP_ROUTE) {
        try { sessionStorage.setItem('od_pending_route', INITIAL_PATH); } catch (_) {}
        return;
      }
      return previousShowAuth.apply(this, arguments);
    };
    showAuth.__od1775RouteRestore = true;
    try { window.showAuth = showAuth; } catch (_) {}
  }

  // Depois do boot online, restaura a rota original.
  if (typeof od42Boot === 'function' && !od42Boot.__od1775RouteRestore) {
    const previousBoot = od42Boot;
    od42Boot = async function od1775Boot(){
      const result = await previousBoot.apply(this, arguments);
      setTimeout(() => restoreRoute(), 60);
      setTimeout(() => restoreRoute(), 450);
      return result;
    };
    od42Boot.__od1775RouteRestore = true;
    try { window.od42Boot = od42Boot; } catch (_) {}
  }

  // Cobre casos em que outro boot já tenha sido chamado antes do wrapper.
  setTimeout(() => restoreRoute(), 900);
  setTimeout(() => restoreRoute(), 1700);
  setTimeout(() => {
    if (!restored && restoring) {
      restoring = false;
      document.documentElement.classList.remove('od1775-restoring-route');
      document.body.classList.remove('od1775-restoring-route');
      if (!currentUser && typeof showAuth === 'function') showAuth();
    }
  }, 3500);
})();
...
/* =========================
   V178.7 - Correção final visual dos nomes dos Atributos
   Motivo:
   Algumas camadas antigas ainda aplicavam ellipsis no nome depois do render.
   Esta correção atua no DOM já renderizado e troca os nomes para abreviações
   oficiais legíveis, sem "...", mantendo significado.
========================= */
(function od1787AttributeNameNoEllipsis(){
  'use strict';
  ...
  function fix(){
    document.body.classList.add('od1787-attribute-name-final');

    document.querySelectorAll('#attributes-grid .attr-card-v2').forEach(card => {
      card.classList.add('od1787-attr-card-final');

      const name = card.querySelector('.attr-name');
      if (!name) return;

      const raw = name.getAttribute('title') || name.dataset.fullText || name.textContent || '';
      const normalized = normalizeText(raw).replace(/\.+$/g, '');

      let finalText = DISPLAY[normalized];

      // Quando já veio cortado como "AGILIDA...", "INTELEC..." ou "PRESEN..."
      if (!finalText) {
        if (normalized.startsWith('agilida')) finalText = 'AGILID.';
        else if (normalized.startsWith('intelec')) finalText = 'INTELECT.';
        else if (normalized.startsWith('presen')) finalText = 'PRESENÇA';
        else if (normalized.startsWith('for')) finalText = 'FORÇA';
        else if (normalized.startsWith('vig')) finalText = 'VIGOR';
      }

      if (finalText) {
        name.textContent = finalText;
        name.dataset.fullText = raw || finalText;
        name.setAttribute('title', raw || finalText);
      }

      name.classList.add('od1787-attr-name-final');
      card.querySelector('.attr-mod')?.classList.add('od1787-attr-mod-final');
      card.querySelector('.attr-head, .od101-attr-top, .od100-attr-head')?.classList.add('od1787-attr-head-final');
    });
  }

  function schedule(){
    clearTimeout(window.__od1787AttrTimer);
    window.__od1787AttrTimer = setTimeout(fix, 30);
  }
...
/* =========================
   V178.8 - Atributos com nome completo
   Corrige a v1.78.7: não pode abreviar. Precisa escrever completo.
========================= */
(function od1788AttributeFullNames(){
  'use strict';
  ...
  function keyFromText(value){
    ...
    if (n.startsWith('agil')) return 'agilidade';
    if (n.startsWith('vig')) return 'vigor';
    if (n.startsWith('intel')) return 'intelecto';
    if (n.startsWith('pres')) return 'presenca';
    return n;
  }

  function applyFullNames(){
    document.body.classList.add('od1788-attr-full-names');

    document.querySelectorAll('#attributes-grid .attr-card-v2').forEach(card => {
      card.classList.add('od1788-attr-card');

      const input = card.querySelector('input[data-attr]');
      const key = input?.dataset?.attr || keyFromText(card.querySelector('.attr-name')?.textContent);
      const name = card.querySelector('.attr-name');
      if (!name) return;

      const full = FULL[key] || FULL[keyFromText(name.textContent)] || name.textContent;
      name.textContent = full;
      name.dataset.fullText = full;
      name.setAttribute('title', full);
      name.classList.add('od1788-attr-name-full');

      card.querySelector('.attr-mod')?.classList.add('od1788-attr-mod');
      card.querySelector('.attr-head, .od101-attr-top, .od100-attr-head, .od1786-attr-title-row')?.classList.add('od1788-attr-head');
    });
  }

  function schedule(){
    clearTimeout(window.__od1788AttrTimer);
    window.__od1788AttrTimer = setTimeout(applyFullNames, 25);
  }

  document.addEventListener('click', event => {
    if (event.target.closest?.('#attributes-grid, [data-od170-toggle], .sheet-tab')) {
      schedule();
      setTimeout(applyFullNames, 120);
    }
  }, true);
  ...
  if (document.readyState === 'loading') document.addEventListener('DOMContentLoaded', applyFullNames, { once: true });
  else applyFullNames();

  setTimeout(applyFullNames, 100);
  setTimeout(applyFullNames, 500);
  setTimeout(applyFullNames, 1200);
  setTimeout(applyFullNames, 2200);
})();
...
/* =========================
   V178.14 - Atributos reconstruídos e estáveis
   - corrige cards quebrados/cobrindo texto;
   - valor volta para dentro do campo;
   - botões ficam alinhados;
   - nomes completos sem corte.
========================= */
(function od17814AttributesStableFinal(){
  'use strict';
  ...
  function getAttrValue(char, key){
    return clampAttr(char?.attrs?.[key] ?? 10);
  }
  ...
  function getAttrMod(value){
    ...
    return Math.floor((clampAttr(value) - 10) / 2);
  }

  function formatModValue(value){
    try {
      if (typeof formatMod === 'function') return formatMod(value);
    } catch (_) {}
    return `${value >= 0 ? '+' : ''}${value}`;
  }

  function updateAttr(key, value, rerender = true){
    const char = getChar();
    if (!char) return;
    const next = clampAttr(value);
    if (typeof updateChar === 'function') {
      updateChar(c => {
        c.attrs = c.attrs || {};
        c.attrs[key] = next;
  ...
  function buildCard(char, key, label){
    ...
    return `
      <article class="od17814-attr-card" data-od17814-card="${esc(key)}">
        <div class="od17814-attr-top">
          <span class="od17814-attr-name" title="${esc(label)}">${esc(label)}</span>
          <span class="od17814-attr-mod">${esc(mod)}</span>
        </div>

        <div class="od17814-attr-controls">
          <span class="od17814-dice-label">D20</span>
          <button type="button" class="od17814-step-btn" data-od17814-step="${esc(key)}" data-dir="-1" aria-label="Diminuir ${esc(label)}">−</button>
          <input type="number" min="1" step="1" inputmode="numeric" class="od17814-value-input" data-od17814-input="${esc(key)}" value="${esc(value)}" aria-label="Valor de ${esc(label)}">
          <button type="button" class="od17814-step-btn" data-od17814-step="${esc(key)}" data-dir="1" aria-label="Aumentar ${esc(label)}">+</button>
        </div>

        <button type="button" class="primary-btn small roll-attr od17814-roll-btn" data-roll-attr="${esc(key)}">D20</button>
      </article>`;
  }

  function render(char = getChar()){
    const grid = document.getElementById('attributes-grid');
    if (!grid || !char) return;
    grid.className = 'attributes-grid od17814-attributes-grid';
    grid.innerHTML = ATTRS.map(([key, label]) => buildCard(char, key, label)).join('');
  }

  function scheduleRender(delay = 30){
    clearTimeout(renderTimer);
    renderTimer = setTimeout(() => render(getChar()), delay);
  }

  function ensureStable(){
    const grid = document.getElementById('attributes-grid');
    if (!grid) return;
    const broken = grid.querySelector('.attr-card-v2, .od1786-attr-card, .od1788-attr-card, .od1777-attr-clean');
    const missing = !grid.querySelector('.od17814-attr-card');
    if (broken || missing || !grid.classList.contains('od17814-attributes-grid')) scheduleRender(20);
  }

  // Override final renderer
  try { renderAttributes = render; } catch (_) {}
  try { window.renderAttributes = render; } catch (_) {}
  window.renderAttributesV17814 = render;

  document.addEventListener('click', event => {
    const step = event.target.closest?.('[data-od17814-step]');
    if (step) {
      event.preventDefault();
      event.stopImmediatePropagation();
      const key = step.dataset.od17814Step;
      const dir = Number(step.dataset.dir || 0);
      const char = getChar();
      if (!char || !key) return;
      updateAttr(key, getAttrValue(char, key) + dir);
      return;
    }

    if (event.target.closest?.('#attributes-grid, [data-od170-toggle], .sheet-tab')) {
      scheduleRender(40);
      setTimeout(() => render(getChar()), 140);
    }
  }, true);

  document.addEventListener('input', event => {
    const input = event.target.closest?.('[data-od17814-input]');
    if (!input) return;
    const key = input.dataset.od17814Input;
    clearTimeout(syncTimer);
    syncTimer = setTimeout(() => updateAttr(key, input.value, false), 120);
  }, true);

  document.addEventListener('change', event => {
    const input = event.target.closest?.('[data-od17814-input]');
    if (!input) return;
    updateAttr(input.dataset.od17814Input, input.value);
  }, true);

  const observer = new MutationObserver(() => ensureStable());
  observer.observe(document.body, { childList: true, subtree: true, attributes: true, attributeFilter: ['class'] });

  if (document.readyState === 'loading') document.addEventListener('DOMContentLoaded', () => render(getChar()), { once: true });
  else render(getChar());

  setTimeout(() => render(getChar()), 100);
  setTimeout(() => render(getChar()), 500);
  setTimeout(() => render(getChar()), 1200);
})();
...
/* =========================
   V180.2 - Hotfix login/autenticação robusta
   Corrige caso em que a senha está correta, mas o login cai em erro por falha
   em carregamentos posteriores de fichas/mesas ou por camada visual recente.
========================= */
(function od1802RobustLogin(){
  'use strict';
  ...
  function cleanNick(value){
    try { if (typeof normalizeNick === 'function') return normalizeNick(value); } catch (_) {}
    return String(value || '').trim().toLowerCase().replace(/\s+/g, '');
  }
  function cleanPass(value){ return String(value || '').trim(); }
  function headers(extra = {}){
    const out = { 'Content-Type': 'application/json', ...(extra || {}) };
    try { if (typeof od42Token === 'function' && od42Token()) out.Authorization = `Bearer ${od42Token()}`; } catch (_) {}
    return out;
  }
  ...
  function mergeUser(user){
    try { if (typeof od42MergeById === 'function') od42MergeById(STORAGE.users, [user]); } catch (_) {}
    try {
  ...
  function setOnlineSession(token, user){
    try { if (typeof od42SetSession === 'function') od42SetSession({ token, user }); } catch (_) {}
    try { if (typeof setSessionValue === 'function') setSessionValue(user.id); } catch (_) {}
    try { localStorage.setItem('od_online_session_v42', JSON.stringify({ token, user })); } catch (_) {}
  }
  ...
  async function refreshAfterLogin(){
    const failures = [];
    try { if (typeof od42RefreshOwnCharacters === 'function') await od42RefreshOwnCharacters(); }
    catch (error) { console.warn('Falha ao carregar fichas após login:', error); failures.push(error); }
    try { if (typeof od42RefreshTables === 'function') await od42RefreshTables(); }
    catch (error) { console.warn('Falha ao carregar mesas após login:', error); failures.push(error); }
    return failures;
  }

  async function robustLogin(nick, password){
    const finalNick = cleanNick(nick);
    const finalPassword = cleanPass(password);
    if (!finalNick) throw new Error('Digite o nick/login.');
    if (!finalPassword) throw new Error('Digite a senha.');

    closeLoaders();

    const data = await api('/api/auth/login', {
      method: 'POST',
      body: JSON.stringify({ nick: finalNick, password: finalPassword })
    });

    const user = mapUser(data.user);
    if (!user?.id || !data.token) throw new Error('Login aceito, mas a sessão veio incompleta.');

    currentUser = user;
    try { window.currentUser = currentUser; } catch (_) {}
    setOnlineSession(data.token, user);
    mergeUser(user);

    // Importante: falha ao carregar fichas/mesas não pode cancelar o login.
    ...
    if (failures.length) {
      console.warn('Login concluído, mas houve falha secundária ao atualizar dados:', failures);
    }
  }

  od42Login = robustLogin;
  login = function od1802Login(nick, password){
    robustLogin(nick, password).catch(error => {
      closeLoaders();
      alert(error?.message || 'Erro ao entrar.');
    });
  };
  try { window.od42Login = od42Login; window.login = login; } catch (_) {}

  function wireLoginForm(){
    const form = $('login-form');
    if (!form || form.dataset.od1802LoginFixed === '1') return;
    form.dataset.od1802LoginFixed = '1';
    form.onsubmit = event => {
      event.preventDefault();
      event.stopPropagation();
      login($('login-nick')?.value, $('login-password')?.value);
      return false;
    };
  }

  if (document.readyState === 'loading') document.addEventListener('DOMContentLoaded', wireLoginForm, { once: true });
  else wireLoginForm();
  setTimeout(wireLoginForm, 150);
  setTimeout(wireLoginForm, 700);
  setTimeout(wireLoginForm, 1500);

  window.od1802RobustLogin = { robustLogin, wireLoginForm, closeLoaders };
})();


/* =========================
   V180.3 - Limpeza das áreas: Início, Personagens e Fichas
   Objetivo:
   - reduzir bugs visuais de área ativa;
   - evitar estado misturado entre login/início/ficha/mesa;
   - manter tema escuro;
   - fechar loaders presos;
   - não usar MutationObserver global infinito.
========================= */
(function od1803AreaCleanup(){
  'use strict';
  ...
  function activeSessionsTab(){
    const tab = safe(() => localStorage.getItem('od71_tab') || localStorage.getItem('od75_tab') || 'home', 'home');
    if (['characters', 'personagens'].includes(tab)) return AREA.CHARACTERS;
    if (['campaigns', 'campanhas'].includes(tab)) return AREA.CAMPAIGNS;
    return AREA.HOME;
  }

  function currentScreenArea(){
    if ($('auth-screen')?.classList.contains('active')) return AREA.AUTH;
    if ($('sessions-screen')?.classList.contains('active')) return activeSessionsTab();
    if ($('app-screen')?.classList.contains('active')) {
      if (typeof accountSheetMode !== 'undefined' && accountSheetMode) return AREA.SHEET;
      if (typeof currentCampaignId !== 'undefined' && currentCampaignId) return AREA.TABLE;
      return AREA.SHEET;
    }
    return AREA.AUTH;
  }

  function ensureOnlyOneScreen(){
    const auth = $('auth-screen');
    const sessions = $('sessions-screen');
    const app = $('app-screen');
    const active = [auth, sessions, app].filter(Boolean).filter(el => el.classList.contains('active'));

    if (active.length <= 1) return;

    const keep = app?.classList.contains('active') ? app : sessions?.classList.contains('active') ? sessions : auth;
    [auth, sessions, app].filter(Boolean).forEach(el => el.classList.toggle('active', el === keep));
  }
  ...
  function setArea(area){
    ...
    document.body.classList.toggle('od1803-login-area', area === AREA.AUTH);
    document.body.classList.toggle('od1803-home-area', area === AREA.HOME);
    document.body.classList.toggle('od1803-characters-area', area === AREA.CHARACTERS);
    document.body.classList.toggle('od1803-campaigns-area', area === AREA.CAMPAIGNS);
    document.body.classList.toggle('od1803-sheet-area', area === AREA.SHEET);
    document.body.classList.toggle('od1803-table-area', area === AREA.TABLE);
  }

  function forceDark(){
    document.body.classList.add('dark-sheet', 'od1803-dark-only');
    document.documentElement.classList.add('od1803-dark-only-root');
    document.body.classList.remove('light-sheet','theme-light','paper-light');
    document.documentElement.classList.remove('light-sheet','theme-light','paper-light');
    document.querySelectorAll('#theme-toggle,#sessions-theme-toggle').forEach(btn => {
      btn.textContent = 'Tema Escuro';
      btn.setAttribute('aria-label', 'Tema escuro fixo');
      btn.classList.add('od1803-disabled-theme-toggle');
    });
  }

  function normalizeInicio(){
    const shell = document.querySelector('.sessions-shell');
    if (shell) shell.classList.add('od1803-sessions-shell');

    const head = document.querySelector('.sessions-head');
    if (head) head.classList.add('od1803-sessions-head');

    document.querySelectorAll('.sessions-actions,.sessions-list-panel,.account-sheets-panel').forEach(el => {
      el.classList.add('od1803-panel');
    });

    const menuBtn = $('sessions-menu-btn');
    if (menuBtn) {
      menuBtn.classList.add('od1803-icon-btn');
      menuBtn.setAttribute('aria-label', 'Abrir menu da conta');
      if (!menuBtn.dataset.od1803Icon) {
        menuBtn.dataset.od1803Icon = '1';
        menuBtn.innerHTML = '<span aria-hidden="true">☰</span>';
      }
    }

    const sheetBtn = $('toggle-account-panel-btn');
    if (sheetBtn) {
      sheetBtn.classList.add('od1803-icon-btn');
      sheetBtn.setAttribute('aria-label', 'Abrir personagens');
      if (!sheetBtn.dataset.od1803Icon) {
        sheetBtn.dataset.od1803Icon = '1';
        sheetBtn.innerHTML = '<span aria-hidden="true">▣</span>';
      }
    }
  }

  function normalizePersonagens(){
    const panel = $('account-sheets-panel');
    const list = $('account-character-list');
    if (panel) panel.classList.add('od1803-characters-panel');
    if (list) {
      list.classList.add('od1803-character-list');
      list.querySelectorAll(':scope > *').forEach(card => card.classList.add('od1803-character-card'));
    }
  }

  function normalizeFicha(){
    const app = $('app-screen');
    if (!app) return;
    app.classList.add('od1803-app-screen');

    const sheet = document.querySelector('.sheet');
    if (sheet) sheet.classList.add('od1803-sheet');

    const sidebar = $('players-sidebar');
    if (sidebar) sidebar.classList.add('od1803-sidebar');

    if (document.body.classList.contains('od1803-sheet-area')) {
      $('campaign-mini-card')?.classList.add('hidden');
      $('master-dashboard')?.classList.add('hidden');
      $('player-dashboard')?.classList.add('hidden');
      $('initiative-panel')?.classList.add('hidden');
    }
  }

  function normalizeTexts(){
    document.querySelectorAll('.campaign-empty,.modal-hint,.subtitle,.eyebrow,small').forEach(el => {
      el.classList.add('od1803-readable-text');
    });
  ...
  function wrap(name, after){
    try {
      ...
      if (typeof original !== 'function' || original.__od1803Wrapped) return;
      const wrapped = function(...args){
        const result = original.apply(this, args);
        setTimeout(() => { safe(after || (() => {}), null); schedule(); }, 0);
        return result;
      };
      wrapped.__od1803Wrapped = true;
      try { window[name] = wrapped; } catch (_) {}
      try { eval(`${name} = wrapped`); } catch (_) {}
    } catch (_) {}
  }

  wrap('showAuth', () => setArea(AREA.AUTH));
  wrap('showSessions', () => setArea(activeSessionsTab()));
  wrap('showApp', () => setArea(currentScreenArea()));
  wrap('initAccountCharacterEditor', () => setArea(AREA.SHEET));
  wrap('initApp', () => setArea(AREA.TABLE));
  wrap('renderAccountCharacterMenu');
  wrap('renderCampaignMenu');
  wrap('loadCharacter');
  wrap('renderCharacterList');

  document.addEventListener('click', event => {
    if (event.target.closest?.('[data-od75-tab],[data-od71-tab],#toggle-account-panel-btn,#sessions-menu-btn,#back-to-sessions-btn,.sheet-tab,#create-account-character-btn,[data-edit-account-character],[data-enter-campaign]')) {
      setTimeout(schedule, 0);
      setTimeout(schedule, 160);
    }

    if (event.target.closest?.('#theme-toggle,#sessions-theme-toggle')) {
      event.preventDefault();
      event.stopImmediatePropagation();
      forceDark();
      schedule();
  ...
  document.addEventListener('input', event => {
    if (event.target.closest?.('#new-campaign-name,#join-campaign-code,#account-character-list,#char-name,.sheet')) {
      schedule();
    }
  }, true);
  ...
  window.od1803AreaCleanup = { run, schedule, forceDark, removeStuckLoaders };
})();


/* =========================
   V180.4 - Hotfix definitivo de Login + áudio único
   Corrige:
   - login correto ficando preso na tela de login;
   - múltiplos handlers disparando login/sons duplicados;
   - som saindo duas vezes por clique.
========================= */
(function od1804LoginAndAudioFix(){
  'use strict';
  ...
  function token(){ try { return typeof od42Token === 'function' ? od42Token() : ''; } catch (_) { return ''; } }
  ...
  function saveSession(tokenValue, user){
    try { if (typeof od42SetSession === 'function') od42SetSession({ token: tokenValue, user }); } catch (_) {}
    try { if (typeof setSessionValue === 'function') setSessionValue(user.id); } catch (_) {}
    try { localStorage.setItem('od_online_session_v42', JSON.stringify({ token: tokenValue, user })); } catch (_) {}
  }
  ...
  async function refreshDataAfterLogin(){
    // Nunca deixa falha secundária cancelar a entrada.
    await Promise.allSettled([
      safe(() => typeof od42RefreshOwnCharacters === 'function' ? od42RefreshOwnCharacters() : Promise.resolve(), Promise.resolve()),
      safe(() => typeof od42RefreshTables === 'function' ? od42RefreshTables() : Promise.resolve(), Promise.resolve())
    ]);
  }

  function forceEnterInicio(){
    const auth = $('auth-screen');
    const sessions = $('sessions-screen');
    const app = $('app-screen');
    ...
    if (typeof currentCampaignId !== 'undefined') currentCampaignId = null;
    if (typeof accountSheetMode !== 'undefined') accountSheetMode = false;
    try { localStorage.removeItem(STORAGE.activeCampaign); } catch (_) {}
    try { localStorage.setItem('od71_tab', 'home'); } catch (_) {}
    try { localStorage.setItem('od75_tab', 'home'); } catch (_) {}

    safe(() => renderAccountCharacterMenu(), null);
    safe(() => renderCampaignMenu(), null);
    safe(() => window.od1803AreaCleanup?.run?.(), null);
  ...
  async function finalLogin(nickValue, passwordValue){
    if (loginBusy) return;
    const thisTicket = ++loginTicket;
    loginBusy = true;
    ...
    try {
      if (!nick) throw new Error('Digite o nick/login.');
      if (!password) throw new Error('Digite a senha.');

      closeLoaders();

      const data = await apiLogin(nick, password);
      if (thisTicket !== loginTicket) return;
      ...
      // Entra primeiro. Carregamentos posteriores não podem segurar a tela de login.
      forceEnterInicio();
  ...
  // Substitui todos os logins antigos por uma única entrada final.
  od42Login = finalLogin;
  login = function od1804Login(nick, password){ finalLogin(nick, password); };
  try { window.od42Login = od42Login; window.login = login; } catch (_) {}

  function wireLoginOnce(){
    const form = $('login-form');
    if (!form) return;
    form.dataset.od1804LoginFixed = '1';
    form.onsubmit = event => {
      event.preventDefault();
      event.stopPropagation();
      finalLogin($('login-nick')?.value, $('login-password')?.value);
      return false;
    };
  ...
  // Sessão já válida não deve ficar presa no login.
  async function restoreIfTokenExists(){
    if (!token()) return;
    try {
      if (!currentUser && typeof od42Api === 'function') {
        const data = await od42Api('/api/auth/me');
        currentUser = mapUser(data.user);
        try { window.currentUser = currentUser; } catch (_) {}
        mergeUser(currentUser);
      }
      forceEnterInicio();
      refreshDataAfterLogin().finally(forceEnterInicio);
    } catch (_) {
      // Se token for inválido, limpa apenas a sessão online e mantém login visível.
      try { if (typeof od42ClearSession === 'function') od42ClearSession(); } catch (_) {}
      closeLoaders();
    }
  }
  ...
  /* ÁUDIO
     A fonte duplicada era: som no pointerdown antigo + som no click novo.
     Mantemos só o sistema novo (od115) e desligamos o som antigo baseado em STORAGE.settings.sound. */
  ...
  // Dedupe global para o sistema de som ativo.
  window.__od1804LastSound = window.__od1804LastSound || { name: '', at: 0 };
  if (window.od115?.beep && !window.od115.beep.__od1804Deduped) {
    const baseBeep = window.od115.beep;
    const deduped = function(name = 'click'){
      const now = Date.now();
      const last = window.__od1804LastSound;
      if (last.name === name && now - last.at < 180) return;
      last.name = name;
      last.at = now;
      return baseBeep(name);
    };
    deduped.__od1804Deduped = true;
    window.od115.beep = deduped;
  }

  wireLoginOnce();
  disableLegacyPointerSound();
  closeLoaders();
  ...
  if (document.readyState === 'loading') {
    document.addEventListener('DOMContentLoaded', () => {
      wireLoginOnce();
      disableLegacyPointerSound();
      restoreIfTokenExists();
    }, { once: true });
  } else {
    restoreIfTokenExists();
  }

  setTimeout(wireLoginOnce, 120);
  setTimeout(wireLoginOnce, 600);
  setTimeout(disableLegacyPointerSound, 120);
...
/* =========================
   ...
   - o modo reduzido é visualização sem edição;
   - o modo expandido é edição contínua, sem resetar ao digitar;
   - botões antigos dos módulos são ocultados ou reaproveitados corretamente.
========================= */
(function od1808CleanEditorsModes(){
  ...
  function fmt(value){ const n = number(value, 0); return Number.isInteger(n) ? String(n) : String(n).replace('.', ','); }
  function weightValue(value){ const n = number(value, .5); if (!Number.isFinite(n) || n <= 0) return .5; return n < 1 ? .5 : Math.round(n * 2) / 2; }
  ...
  function invReduced(char = cchar()){ return !!char?.simpleInventoryCompact; }
  function spellsReduced(char = cchar()){ return !!char?.spellsCompact; }
  function abilitiesReduced(char = cchar()){ return !!char?.abilitiesCompact; }

  function scheduleServerSave(delay = 850){
    if (typeof queueSave === 'function') safe(() => queueSave(), null);
    clearTimeout(saveTimer);
    saveTimer = setTimeout(() => {
      syncAllFromDOM({ renderSummary: true });
  ...
  function cleanupSkillsModule(){
    const module = document.querySelector('.od170-module[data-od170-key="pericias-lista"]');
    if (!module) return;
    module.classList.remove('od170-collapsed');
    module.classList.add('od1810-skills-no-collapse');
    module.querySelector(':scope > .od1715-collapse-summary')?.remove();
    const btn = module.querySelector(':scope > .od170-module-head [data-od170-toggle]');
    if (btn) btn.remove();
    const body = module.querySelector(':scope > .od170-module-body');
    if (body) body.style.removeProperty('display');
  }
  function cleanupSessionProfileFinal(){
    document.querySelectorAll('.od90-profile-card,[data-od90-profile-card],.session-profile-card,.account-profile-card,.menu-profile-card,#session-profile-card,#account-menu-profile').forEach(el => el.remove());
  }
  function applyBlockInventoryFinal(active){
    const char = setChar(saved => { saved.blockInventoryMode = !!active; });
    const blockPanel = $('block-inventory-panel');
    const simplePanel = $('simple-inventory-panel');
    blockPanel?.classList.toggle('active', !!active);
    simplePanel?.classList.toggle('active', !active);
    $('tab-equipamentos')?.classList.toggle('block-inventory-active', !!active);
    document.body.classList.toggle('block-inventory-active', !!active);
    const module = document.querySelector('.od170-module[data-od170-key="equip-modular"]');
    if (module) {
      module.hidden = !active;
      module.classList.toggle('od170-collapsed', !active);
    }
    const btn = $('block-inventory-toggle');
    if (btn) {
      btn.textContent = active ? 'Trocar para Texto' : 'Trocar para Block Inventory';
      btn.setAttribute('aria-pressed', active ? 'true' : 'false');
    }
    renderInventorySummary(char);
    if (active) safe(() => renderBlockInventory(char), null);
    scheduleServerSave(120);
  }
  ...
  function summaryValuesFromDOM(){
    const char = cchar();
    return {
      money: $('money')?.value ?? char?.money ?? '0',
      max: number($('weight-max')?.value ?? char?.weightMax ?? 0, 0)
    };
  }
  function updateWeightStatusOnly(char = cchar()){
    const status = $('weight-status');
    if (!status || !char) return;
    const items = ensureItems(char);
    const weight = currentWeight(items);
    const max = number(($('weight-max')?.value ?? char.weightMax ?? 0), 0);
    status.className = 'weight-status-line od1808-weight-status';
    status.textContent = `${weight <= max || !max ? 'Peso dentro do limite.' : 'Peso acima do limite.'} Peso ${fmt(weight)} / ${fmt(max)}.`;
  }
  function renderInventorySummary(char = cchar()){
    ...
    grid.className = 'equipment-grid inventory-summary-grid od1808-summary-grid';
    grid.innerHTML = `
      <label class="od1808-summary-card"><span>Dinheiro</span><input id="money" type="text" value="${esc(money)}" autocomplete="off"></label>
      <label class="od1808-summary-card od1808-readonly"><span>Peso Atual</span><input id="weight-current" type="text" value="${esc(fmt(weight))}" readonly></label>
      <label class="od1808-summary-card"><span>Peso Máximo</span><input id="weight-max" type="number" min="0" step="0.5" value="${esc(max)}"></label>`;
    updateWeightStatusOnly(char);
  }
  ...
  function readInventoryDOM(){
    ...
    if (!list || invReduced(char)) return ensureItems(char).map(cloneItem);
    if (!list.classList.contains('od1808-inventory-list')) return ensureItems(char).map(cloneItem);
    return [...list.querySelectorAll('.od1808-inv-card-edit')].map(card => cloneItem({
      id: card.dataset.itemId,
      name: card.querySelector('[data-od1808-inv="name"]')?.value ?? '',
      weight: card.querySelector('[data-od1808-inv="weight"]')?.value ?? .5,
      uses: card.querySelector('[data-od1808-inv="uses"]')?.value ?? 0,
      desc: card.querySelector('[data-od1808-inv="desc"]')?.value ?? ''
    }));
  }
  function persistInventory(items, renderSummary = false){
    const normalized = (items || []).map(cloneItem);
    const weight = currentWeight(normalized);
    const values = summaryValuesFromDOM();
    const char = setChar(saved => {
      saved.inventoryItems = normalized;
      saved.weightCurrent = weight;
      saved.weightMax = values.max;
      saved.money = String(values.money);
    ...
    else updateWeightStatusOnly(char);
  }
  function renderInventoryHeader(char = cchar()){
    ...
    if (!head) return;
    panel.classList.add('od1808-inventory-panel');
    head.classList.add('od1808-editor-head');
    const reduced = invReduced(char);
    ...
    if (toggle) {
      ...
      toggle.classList.add('od1808-head-btn');
      toggle.hidden = false;
    ...
    if (add) {
      ...
      add.classList.add('od1808-head-btn','od1808-add-btn');
      add.hidden = reduced;
  ...
  function inventoryViewCard(item){
    return `<article class="od1808-inv-card-view" data-item-id="${esc(item.id)}">
      <div class="od1808-card-actions">
      ...
      <div class="od1808-view-body">
        <h4>${esc(item.name || 'Item')}</h4>
        <div class="od1808-tags"><span>Peso: ${esc(fmt(item.weight))}</span><span>Usos: ${esc(item.uses || 0)}</span></div>
        <p>${esc(item.desc || 'Sem descrição.')}</p>
      </div>
    </article>`;
  }
  function inventoryEditCard(item, index, total){
    return `<article class="od1808-inv-card-edit" data-item-id="${esc(item.id)}">
      <div class="od1808-card-actions">
        <button class="ghost-btn small" data-transfer-simple-item="${esc(item.id)}" type="button">↔ Transferir</button>
        <button class="ghost-btn small" data-drop-simple-item="${esc(item.id)}" type="button">▣ Enviar para Drop</button>
      </div>
      <label class="od1808-field"><span>Nome do item</span><div class="od1808-title-row"><input data-od1808-inv="name" data-inv-field="name" type="text" value="${esc(item.name || '')}" placeholder="Nome do item"><button class="mini-danger-btn od1808-delete" data-od1808-remove-inventory="${esc(item.id)}" type="button">×</button></div></label>
      <label class="od1808-field"><span>Descrição</span><textarea data-od1808-inv="desc" data-inv-field="desc" rows="6" placeholder="Descrição do item...">${esc(item.desc || '')}</textarea></label>
      <div class="od1808-inv-bottom">
        <label class="od1808-box"><span>Peso</span><div class="od1808-stepper"><button data-od1808-step="weight" data-dir="-1" type="button">−</button><input data-od1808-inv="weight" data-inv-field="weight" type="text" inputmode="decimal" value="${esc(fmt(item.weight))}"><button data-od1808-step="weight" data-dir="1" type="button">+</button></div></label>
        <label class="od1808-box"><span>Usos</span><div class="od1808-stepper"><button data-od1808-step="uses" data-dir="-1" type="button">−</button><input data-od1808-inv="uses" data-inv-field="uses" type="number" min="0" step="1" value="${esc(item.uses || 0)}"><button data-od1808-step="uses" data-dir="1" type="button">+</button></div></label>
        <div class="od1808-order"><span>Ordem</span><div><button class="ghost-btn small" data-od1808-move-inventory="${esc(item.id)}" data-dir="-1" type="button" ${index === 0 ? 'disabled' : ''} aria-label="Subir item">↑</button><button class="ghost-btn small" data-od1808-move-inventory="${esc(item.id)}" data-dir="1" type="button" ${index >= total - 1 ? 'disabled' : ''} aria-label="Descer item">↓</button></div></div>
      </div>
    </article>`;
  }
  function renderInventory(char = cchar()){
    ...
    try {
      ...
      renderInventorySummary(char);
      list.className = `simple-inventory-list od1808-inventory-list ${reduced ? 'od1808-reduced' : 'od1808-expanded'}`;
      if (!items.length) list.innerHTML = '<div class="simple-inventory-empty od1808-empty">Nenhum item adicionado.</div>';
      else list.innerHTML = reduced ? items.map(inventoryViewCard).join('') : items.map((item,i)=>inventoryEditCard(item,i,items.length)).join('');
      applyBlockInventoryVisibility(char);
    } finally { rendering = false; }
  }
  function addInventoryItem(){
    persistInventory(readInventoryDOM().concat([{ id: uidSafe('inv'), name: 'Novo item', weight: .5, uses: 0, desc: '' }]), true);
    setChar(saved => { saved.simpleInventoryCompact = false; });
    renderInventory();
    scheduleServerSave(80);
  }
  function removeInventoryItem(id){ persistInventory(readInventoryDOM().filter(item => String(item.id) !== String(id)), true); renderInventory(); scheduleServerSave(80); }
  function moveInventory(id, dir){
    ...
    persistInventory(items, true); renderInventory(); scheduleServerSave(80);
  }
  function stepInventory(btn){
    const card = btn.closest('.od1808-inv-card-edit');
    const field = btn.dataset.od1808Step;
    const input = card?.querySelector(`[data-od1808-inv="${field}"]`);
    if (!input) return;
    const dir = Number(btn.dataset.dir || 0);
    if (field === 'weight') {
      const current = weightValue(input.value);
      input.value = fmt(dir > 0 ? (current < 1 ? 1 : current + .5) : (current <= .5 ? .5 : current - .5));
    } else input.value = String(Math.max(0, Math.round(number(input.value,0) + dir)));
    persistInventory(readInventoryDOM(), false);
    scheduleServerSave(550);
  }
  function applyBlockInventoryVisibility(char = cchar()){
    const active = !!char?.blockInventoryMode;
    $('block-inventory-panel')?.classList.toggle('active', active);
    $('simple-inventory-panel')?.classList.toggle('active', !active);
    $('tab-equipamentos')?.classList.toggle('block-inventory-active', active);
    document.body.classList.toggle('block-inventory-active', active);
    const module = document.querySelector('.od170-module[data-od170-key="equip-modular"]');
    if (module) {
  ...
  function readSpellsDOM(){
    ...
    if (!list || !list.classList.contains('od1808-spells-list') || spellsReduced(char)) return ensureSpells(char).map(spell => ({...spell}));
    return [...list.querySelectorAll('.od1808-spell-card-edit')].map(card => ({
      name: card.querySelector('.spell-name')?.value || '',
      circle: card.querySelector('.spell-circle')?.value || '',
      exec: card.querySelector('.spell-exec')?.value || '',
      range: card.querySelector('.spell-range')?.value || '',
  ...
  function smallField(label, cls, placeholder, value, readonly = false){ return `<label class="od1808-small-field"><span>${label}</span><input class="${cls}" placeholder="${placeholder}" value="${esc(value || '')}" ${readonly ? 'readonly tabindex="-1"' : ''}></label>`; }
  function spellSources(spell){
    return `<div class="od1808-source-fields" aria-hidden="true">
      <input class="spell-name" value="${esc(spell.name || '')}" readonly><input class="spell-circle" value="${esc(spell.circle || '')}" readonly><input class="spell-exec" value="${esc(spell.exec || '')}" readonly><input class="spell-range" value="${esc(spell.range || '')}" readonly><input class="spell-cost" value="${esc(spell.cost || spell.pe || '')}" readonly><input class="spell-components" value="${esc(spell.components || '')}" readonly><textarea class="spell-description" readonly>${esc(spell.description || spell.effect || '')}</textarea><textarea class="spell-upgrades" readonly>${esc(spell.upgrades || '')}</textarea>
    </div>`;
  }
  function spellViewCard(spell = {}){
    return `<article class="mini-card spell-card od1808-editor-card od1808-spell-card-view">
      ${spellSources(spell)}
      <div class="od1808-view-title">${esc(spell.name || 'Magia')}</div>
      <div class="od1808-view-tags"><span>Círculo: ${esc(spell.circle || '-')}</span><span>Execução: ${esc(spell.exec || '-')}</span><span>Alcance: ${esc(spell.range || '-')}</span><span>Custo: ${esc(spell.cost || spell.pe || '-')}</span><span>Comp.: ${esc(spell.components || '-')}</span></div>
      <div class="od1808-view-text"><strong>Descrição</strong><p>${esc(spell.description || spell.effect || 'Sem descrição.')}</p></div>
      <div class="od1808-view-text"><strong>Aprimoramentos</strong><p>${esc(spell.upgrades || 'Sem aprimoramentos.')}</p></div>
    </article>`;
  }
  function spellEditCard(spell = {}, index = 0, total = 1){
    return `<article class="mini-card spell-card od1808-editor-card od1808-spell-card-edit" data-index="${index}">
      <label class="od1808-field od1808-title-field"><input class="spell-name od1808-title-input" placeholder="Nome da magia" value="${esc(spell.name || '')}"></label>
      <div class="od1808-small-grid">
        ...
        <div class="od1808-order od1808-card-order"><span>Ordem</span><div><button class="ghost-btn small" data-od1808-move-spell="${index}" data-dir="-1" type="button" ${index === 0 ? 'disabled' : ''}>↑</button><button class="ghost-btn small" data-od1808-move-spell="${index}" data-dir="1" type="button" ${index >= total - 1 ? 'disabled' : ''}>↓</button></div></div>
      </div>
      <label class="od1808-field"><span>Descrição</span><textarea class="spell-description od1808-big-text" rows="8" placeholder="Descrição da magia">${esc(spell.description || spell.effect || '')}</textarea></label>
      <label class="od1808-field"><span>Aprimoramentos</span><textarea class="spell-upgrades od1808-mid-text" rows="6" placeholder="Aprimoramentos, versões superiores ou gastos extras">${esc(spell.upgrades || '')}</textarea></label>
      <div class="od1808-footer"><button class="danger-btn small od1808-remove" data-od1808-remove-spell="${index}" type="button">🗑 Remover</button></div>
    </article>`;
  }
  function updateSpellHeader(char = cchar()){
    ...
    module?.classList.remove('od170-collapsed');
    const btn = module?.querySelector('[data-od170-toggle]');
    if (btn) { btn.textContent = spellsReduced(char) ? 'Expandir' : 'Reduzir'; btn.classList.add('od1808-mode-toggle'); }
    const add = $('add-spell');
    if (add) { add.hidden = spellsReduced(char); add.disabled = spellsReduced(char); }
  }
  function renderSpellsFinal(char = cchar()){
    const list = $('spells-list');
    if (!list || !char) return;
    const spells = ensureSpells(char);
    const reduced = spellsReduced(char);
    updateSpellHeader(char);
    list.className = `cards-list od1808-editor-list od1808-spells-list ${reduced ? 'od1808-reduced' : 'od1808-expanded'}`;
    list.innerHTML = spells.length ? (reduced ? spells.map(spellViewCard).join('') : spells.map((spell,i)=>spellEditCard(spell,i,spells.length)).join('')) : '<div class="simple-inventory-empty od1808-empty">Nenhuma magia adicionada.</div>';
  }
  function addSpellFinal(spell = {}){ const spells = readSpellsDOM(); spells.push({ name: spell.name || '', circle: spell.circle || '', exec: spell.exec || '', range: spell.range || '', cost: spell.cost || '', components: spell.components || '', description: spell.description || '', upgrades: spell.upgrades || '' }); persistSpells(spells); setChar(c=>{c.spellsCompact=false;}); renderSpellsFinal(); scheduleServerSave(80); }
  function removeSpell(index){ const spells = readSpellsDOM(); spells.splice(Number(index), 1); persistSpells(spells); renderSpellsFinal(); scheduleServerSave(80); }
  function moveSpell(index, dir){ const spells = readSpellsDOM(); const from=Number(index), to=from+Number(dir); if (to<0||to>=spells.length) return; const [x]=spells.splice(from,1); spells.splice(to,0,x); persistSpells(spells); renderSpellsFinal(); scheduleServerSave(80); }

  /* ---------- Habilidades ---------- */
  function normalizeAbility(ability = {}){
    const costAmount = Math.max(0, Math.round(number(ability.costAmount ?? String(ability.cost || '').match(/\d+/)?.[0] ?? 0, 0)));
    const costResource = ability.costResource || (String(ability.cost || '').includes('PV') ? 'PV' : 'PE');
    return { name: ability.name || '', costAmount, costResource, cost: costAmount > 0 ? `${costAmount} ${costResource}` : '', bonus: ability.bonus || '', action: ability.action || 'Padrão', description: ability.description || ability.desc || '' };
  }
  function readAbilitiesDOMFinal(){
    const char = cchar();
    const list = $('abilities-list');
    if (!list || !list.classList.contains('od1808-abilities-list') || abilitiesReduced(char)) return ensureAbilities(char).map(normalizeAbility);
    return [...list.querySelectorAll('.od1808-ability-card-edit')].map(card => normalizeAbility({
      name: card.querySelector('.ability-name')?.value || '',
      costAmount: card.querySelector('.ability-cost-amount')?.value || 0,
      costResource: card.querySelector('.ability-cost-resource')?.value || 'PE',
      bonus: card.querySelector('.ability-bonus')?.value || '',
      action: card.querySelector('.ability-action')?.value || 'Padrão',
      description: card.querySelector('.ability-description')?.value || ''
    }));
  }
  function persistAbilities(abilities){ setChar(saved => { saved.abilities = Array.isArray(abilities) ? abilities.map(normalizeAbility) : []; saved.abilitiesNotes = saved.abilities.map(item => `${item.name || 'Habilidade'} | Custo: ${item.cost || '0 ' + (item.costResource || 'PE')} | Bônus: ${item.bonus || '-'} | Ação: ${item.action || 'Padrão'} | ${item.description || ''}`).join('\n'); }); }
  function abilitySources(a){
    return `<div class="od1808-source-fields" aria-hidden="true"><input class="ability-name" value="${esc(a.name || '')}" readonly><input class="ability-cost-amount" value="${esc(a.costAmount || 0)}" readonly><select class="ability-cost-resource"><option value="${esc(a.costResource || 'PE')}" selected>${esc(a.costResource || 'PE')}</option></select><input class="ability-bonus" value="${esc(a.bonus || '')}" readonly><select class="ability-action"><option value="${esc(a.action || 'Padrão')}" selected>${esc(a.action || 'Padrão')}</option></select><textarea class="ability-description" readonly>${esc(a.description || '')}</textarea></div>`;
  }
  function abilityViewCard(ability = {}){
    const a = normalizeAbility(ability);
    return `<article class="mini-card ability-card od1808-editor-card od1808-ability-card-view">
      ${abilitySources(a)}
      <div class="od1808-view-title">${esc(a.name || 'Habilidade')}</div>
      <div class="od1808-view-tags"><span>Custo: ${esc(a.cost || 'Sem custo')}</span><span>Ação: ${esc(a.action || '-')}</span><span>Bônus: ${esc(a.bonus || '-')}</span></div>
      <div class="od1808-view-text"><strong>Descrição</strong><p>${esc(a.description || 'Sem descrição.')}</p></div>
    </article>`;
  }
  function abilityEditCard(ability = {}, index = 0, total = 1){
    const a = normalizeAbility(ability);
    return `<article class="mini-card ability-card od1808-editor-card od1808-ability-card-edit" data-index="${index}">
      <label class="od1808-field od1808-title-field"><input class="ability-name od1808-title-input" placeholder="Nome da habilidade" value="${esc(a.name || '')}"></label>
      <div class="od1808-small-grid">
        <label class="od1808-small-field"><span>Custo</span><div class="od1808-stepper"><button data-od1808-ability-cost="-1" type="button">−</button><input class="ability-cost-amount" type="number" min="0" step="1" value="${esc(a.costAmount)}"><button data-od1808-ability-cost="1" type="button">+</button></div></label>
        <label class="od1808-small-field"><span>Tipo de custo</span><select class="ability-cost-resource"><option value="PE" ${a.costResource === 'PE' ? 'selected' : ''}>PE</option><option value="PV" ${a.costResource === 'PV' ? 'selected' : ''}>PV</option></select></label>
        <label class="od1808-small-field"><span>Bônus</span><input class="ability-bonus" placeholder="Ex: +2, +1d6" value="${esc(a.bonus || '')}"></label>
        <label class="od1808-small-field"><span>Ação</span><select class="ability-action"><option value="Padrão" ${a.action === 'Padrão' ? 'selected' : ''}>Padrão</option><option value="Movimento" ${a.action === 'Movimento' ? 'selected' : ''}>Movimento</option><option value="Reação" ${a.action === 'Reação' ? 'selected' : ''}>Reação</option><option value="Livre" ${a.action === 'Livre' ? 'selected' : ''}>Livre</option></select></label>
        <div class="od1808-order od1808-card-order"><span>Ordem</span><div><button class="ghost-btn small" data-od1808-move-ability="${index}" data-dir="-1" type="button" ${index === 0 ? 'disabled' : ''}>↑</button><button class="ghost-btn small" data-od1808-move-ability="${index}" data-dir="1" type="button" ${index >= total - 1 ? 'disabled' : ''}>↓</button></div></div>
      </div>
      <label class="od1808-field"><span>Descrição</span><textarea class="ability-description od1808-big-text" rows="8" placeholder="Descrição da habilidade">${esc(a.description || '')}</textarea></label>
      <div class="od1808-footer"><button class="primary-btn small od1808-use" data-od1808-use-ability="${index}" type="button">Usar</button><button class="danger-btn small od1808-remove" data-od1808-remove-ability="${index}" type="button">🗑 Remover</button></div>
    </article>`;
  }
  function updateAbilityHeader(char = cchar()){
    ...
    module?.classList.remove('od170-collapsed');
    const btn = module?.querySelector('[data-od170-toggle]');
    if (btn) { btn.textContent = abilitiesReduced(char) ? 'Expandir' : 'Reduzir'; btn.classList.add('od1808-mode-toggle'); }
    const add = $('add-ability');
    if (add) { add.hidden = abilitiesReduced(char); add.disabled = abilitiesReduced(char); }
  }
  function renderAbilitiesFinal(char = cchar()){
    const list = $('abilities-list');
    if (!list || !char) return;
    const abilities = ensureAbilities(char).map(normalizeAbility);
    const reduced = abilitiesReduced(char);
    updateAbilityHeader(char);
    list.className = `cards-list ability-cards-list od1808-editor-list od1808-abilities-list ${reduced ? 'od1808-reduced' : 'od1808-expanded'}`;
    list.innerHTML = abilities.length ? (reduced ? abilities.map(abilityViewCard).join('') : abilities.map((ability,i)=>abilityEditCard(ability,i,abilities.length)).join('')) : '<div class="simple-inventory-empty od1808-empty">Nenhuma habilidade adicionada.</div>';
  }
  function addAbilityFinal(ability = {}){ const abilities = readAbilitiesDOMFinal(); abilities.push(normalizeAbility(ability)); persistAbilities(abilities); setChar(c=>{c.abilitiesCompact=false;}); renderAbilitiesFinal(); scheduleServerSave(80); }
  function removeAbility(index){ const abilities = readAbilitiesDOMFinal(); abilities.splice(Number(index), 1); persistAbilities(abilities); renderAbilitiesFinal(); scheduleServerSave(80); }
  function moveAbility(index, dir){ const abilities = readAbilitiesDOMFinal(); const from=Number(index), to=from+Number(dir); if (to<0||to>=abilities.length) return; const [x]=abilities.splice(from,1); abilities.splice(to,0,x); persistAbilities(abilities); renderAbilitiesFinal(); scheduleServerSave(80); }
  function abilityCostStep(btn){ const input = btn.closest('.od1808-stepper')?.querySelector('.ability-cost-amount'); if (!input) return; input.value = String(Math.max(0, Math.round(number(input.value,0) + Number(btn.dataset.od1808AbilityCost || 0)))); persistAbilities(readAbilitiesDOMFinal()); scheduleServerSave(550); }

  /* ---------- sincronização ---------- */
  function syncSummaryOnly(){
    const values = summaryValuesFromDOM();
    const char = setChar(saved => { saved.money = String(values.money); saved.weightMax = values.max; saved.weightCurrent = currentWeight(ensureItems(saved)); });
    updateWeightStatusOnly(char);
  }
  function syncAllFromDOM(options = {}){
    const char = cchar(); if (!char) return;
    if ($('simple-inventory-list')?.classList.contains('od1808-inventory-list')) persistInventory(readInventoryDOM(), !!options.renderSummary);
    if ($('spells-list')?.classList.contains('od1808-spells-list')) persistSpells(readSpellsDOM());
    if ($('abilities-list')?.classList.contains('od1808-abilities-list')) persistAbilities(readAbilitiesDOMFinal());
    if ($('money') || $('weight-max')) syncSummaryOnly();
  }
  function renderAll(char = cchar()){
    ...
    cleanupInventorySummaryToggle();
    cleanupSkillsModule();
    cleanupSessionProfileFinal();
    renderInventory(char);
    renderSpellsFinal(char);
    renderAbilitiesFinal(char);
    enforceAttributeCollapse();
  }
  function scheduleRenderAll(){ clearTimeout(renderTimer); renderTimer = setTimeout(() => renderAll(cchar()), 80); }

  /* ---------- Atributos reduzidos ---------- */
  function enforceAttributeCollapse(){
    const module = document.querySelector('.od170-module[data-od170-key="resumo-atributos"]');
    if (!module) return;
    const collapsed = module.classList.contains('od170-collapsed');
    const body = module.querySelector(':scope > .od170-module-body');
    const summary = module.querySelector(':scope > .od1715-attr-summary');
    if (body) body.style.setProperty('display', collapsed ? 'none' : '', collapsed ? 'important' : '');
    if (summary) summary.style.setProperty('display', collapsed ? 'grid' : 'none', 'important');
  }
  ...
  document.addEventListener('click', event => {
    ...
    const blockToggle = target.closest?.('#block-inventory-toggle');
    if (blockToggle) {
      event.preventDefault();
      event.stopImmediatePropagation();
      syncAllFromDOM({ renderSummary: true });
      const char = cchar();
      const next = !char?.blockInventoryMode;
      applyBlockInventoryFinal(next);
      return;
    }

    const oldProfile = target.closest?.('.od90-profile-card,[data-od90-profile-card],.session-profile-card,.account-profile-card,.menu-profile-card,#session-profile-card,#account-menu-profile');
    if (oldProfile) {
      event.preventDefault();
      event.stopImmediatePropagation();
      cleanupSessionProfileFinal();
    ...
    const attrToggle = target.closest?.('.od170-module[data-od170-key="resumo-atributos"] [data-od170-toggle]');
    if (attrToggle) setTimeout(enforceAttributeCollapse, 0);

    const invModuleToggle = target.closest?.('.od170-module[data-od170-key="equip-lista-simples"] [data-od170-toggle]');
    if (invModuleToggle) { event.preventDefault(); event.stopImmediatePropagation(); document.querySelector('.od170-module[data-od170-key="equip-lista-simples"]')?.classList.remove('od170-collapsed'); return; }

    const invToggle = target.closest?.('#simple-inventory-compact-toggle');
    if (invToggle) { event.preventDefault(); event.stopImmediatePropagation(); syncAllFromDOM({ renderSummary: true }); setChar(saved => { saved.simpleInventoryCompact = !saved.simpleInventoryCompact; }); renderInventory(); scheduleServerSave(80); return; }
    ...
    const remInv = target.closest?.('[data-od1808-remove-inventory]');
    if (remInv) { event.preventDefault(); event.stopImmediatePropagation(); removeInventoryItem(remInv.dataset.od1808RemoveInventory); return; }
    const moveInv = target.closest?.('[data-od1808-move-inventory]');
    if (moveInv) { event.preventDefault(); event.stopImmediatePropagation(); moveInventory(moveInv.dataset.od1808MoveInventory, moveInv.dataset.dir); return; }
    const invStep = target.closest?.('[data-od1808-step]');
    if (invStep) { event.preventDefault(); event.stopImmediatePropagation(); stepInventory(invStep); return; }

    const spellToggle = target.closest?.('.od170-module[data-od170-key="magias-lista"] [data-od170-toggle]');
    if (spellToggle) { event.preventDefault(); event.stopImmediatePropagation(); syncAllFromDOM(); setChar(saved => { saved.spellsCompact = !saved.spellsCompact; }); renderSpellsFinal(); scheduleServerSave(80); return; }
    ...
    const remSpell = target.closest?.('[data-od1808-remove-spell]');
    if (remSpell) { event.preventDefault(); event.stopImmediatePropagation(); removeSpell(remSpell.dataset.od1808RemoveSpell); return; }
    const moveSp = target.closest?.('[data-od1808-move-spell]');
    if (moveSp) { event.preventDefault(); event.stopImmediatePropagation(); moveSpell(moveSp.dataset.od1808MoveSpell, moveSp.dataset.dir); return; }

    const abilityToggle = target.closest?.('.od170-module[data-od170-key="habilidades-lista"] [data-od170-toggle]');
    if (abilityToggle) { event.preventDefault(); event.stopImmediatePropagation(); syncAllFromDOM(); setChar(saved => { saved.abilitiesCompact = !saved.abilitiesCompact; saved.abilityDescriptionsHidden = false; }); renderAbilitiesFinal(); scheduleServerSave(80); return; }
    const oldAbilityToggle = target.closest?.('#toggle-ability-desc');
    if (oldAbilityToggle) { event.preventDefault(); event.stopImmediatePropagation(); return; }
    ...
    const remAbility = target.closest?.('[data-od1808-remove-ability]');
    if (remAbility) { event.preventDefault(); event.stopImmediatePropagation(); removeAbility(remAbility.dataset.od1808RemoveAbility); return; }
    const moveAb = target.closest?.('[data-od1808-move-ability]');
    if (moveAb) { event.preventDefault(); event.stopImmediatePropagation(); moveAbility(moveAb.dataset.od1808MoveAbility, moveAb.dataset.dir); return; }
    const abCost = target.closest?.('[data-od1808-ability-cost]');
    if (abCost) { event.preventDefault(); event.stopImmediatePropagation(); abilityCostStep(abCost); return; }
    const useAb = target.closest?.('[data-od1808-use-ability]');
    if (useAb) { event.preventDefault(); event.stopImmediatePropagation(); persistAbilities(readAbilitiesDOMFinal()); safe(() => useAbilityCard(useAb.closest('.ability-card')), null); scheduleServerSave(550); return; }

    const anyModuleToggle = target.closest?.('[data-od170-toggle]');
    const isAttrToggle = target.closest?.('.od170-module[data-od170-key="resumo-atributos"] [data-od170-toggle]');
    const isSkillToggle = target.closest?.('.od170-module[data-od170-key="pericias-lista"] [data-od170-toggle]');
    if (isSkillToggle) { setTimeout(cleanupSkillsModule, 0); return; }
    if (isAttrToggle) { setTimeout(enforceAttributeCollapse, 0); return; }
    if (target.closest?.('.sheet-tab') || anyModuleToggle) setTimeout(() => { renderAll(cchar()); enforceAttributeCollapse(); cleanupSkillsModule(); }, 90);
  }, true);

  document.addEventListener('input', event => {
    const t = event.target;
    if (t.closest?.('.od1808-inv-card-edit')) {
      persistInventory(readInventoryDOM(), false);
      scheduleServerSave(900);
      return;
    }
    if (t.closest?.('.od1808-spell-card-edit')) { persistSpells(readSpellsDOM()); scheduleServerSave(900); return; }
    if (t.closest?.('.od1808-ability-card-edit')) { persistAbilities(readAbilitiesDOMFinal()); scheduleServerSave(900); return; }
    if (t.closest?.('#money,#weight-max')) { syncSummaryOnly(); scheduleServerSave(900); return; }
  }, true);

  document.addEventListener('change', event => {
    const t = event.target;
    if (t.closest?.('.od1808-inv-card-edit')) { persistInventory(readInventoryDOM(), true); scheduleServerSave(300); return; }
    if (t.closest?.('.od1808-spell-card-edit')) { persistSpells(readSpellsDOM()); scheduleServerSave(300); return; }
    if (t.closest?.('.od1808-ability-card-edit')) { persistAbilities(readAbilitiesDOMFinal()); scheduleServerSave(300); return; }
    if (t.closest?.('#money,#weight-max')) { syncSummaryOnly(); renderInventorySummary(cchar()); scheduleServerSave(300); return; }
  }, true);
  ...
  window.od1808CleanEditorsModes = { renderAll, renderInventory, renderSpellsFinal, renderAbilitiesFinal, readInventoryDOM, readSpellsDOM, readAbilitiesDOMFinal };
})();


/* =========================
   V180.10 - Ajustes de estabilidade: atributos, block inventory, perícias e botão fichas
========================= */
(function od1810FinalFixes(){
  'use strict';
  ...
  function cleanupSkillsModule(){
    ...
    module.classList.add('od1810-skills-no-collapse');
    module.querySelector(':scope > .od1715-collapse-summary')?.remove();
    module.querySelector(':scope > .od170-module-head [data-od170-toggle]')?.remove();
    const body = module.querySelector(':scope > .od170-module-body');
    if (body) body.style.removeProperty('display');
  }

  function cleanupSessionProfile(){
    document.querySelectorAll('.od90-profile-card,[data-od90-profile-card],.session-profile-card,.account-profile-card,.menu-profile-card,#session-profile-card,#account-menu-profile').forEach(el => el.remove());
  }

  function refreshSheetButton(){
    const buttons = [$('#toggle-account-panel-btn'), $('#sidebar-dock-btn'), ...document.querySelectorAll('.sessions-sheet-toggle')].filter(Boolean);
    buttons.forEach(btn => {
      btn.classList.add('od1810-sheet-btn-final');
      btn.classList.remove('od90-sheet-stable-btn');
      btn.title = 'Minhas Fichas';
      btn.setAttribute('aria-label', 'Minhas Fichas');
      if (!btn.querySelector('.od1810-sheet-icon')) btn.innerHTML = '<span class="od1810-sheet-icon" aria-hidden="true"><span></span></span>';
    });
  }

  function applyBlockMode(active){
    const c = char();
    if (c) {
      if (typeof updateChar === 'function') updateChar(saved => { saved.blockInventoryMode = !!active; });
      else c.blockInventoryMode = !!active;
    }
    $('#block-inventory-panel')?.classList.toggle('active', !!active);
    $('#simple-inventory-panel')?.classList.toggle('active', !active);
    $('#tab-equipamentos')?.classList.toggle('block-inventory-active', !!active);
    ...
    safe(() => typeof renderInventorySummary === 'function' && renderInventorySummary(char()), null);
    if (active) safe(() => typeof renderBlockInventory === 'function' && renderBlockInventory(char()), null);
    safe(() => typeof queueSave === 'function' && queueSave(), null);
    setTimeout(() => safe(() => typeof saveCurrentCharacter === 'function' && saveCurrentCharacter(), null), 90);
  }

  document.addEventListener('click', event => {
    const btn = event.target.closest?.('#block-inventory-toggle');
    if (btn) {
      event.preventDefault();
      event.stopImmediatePropagation();
      applyBlockMode(!char()?.blockInventoryMode);
      return;
    }

    if (event.target.closest?.('.od170-module[data-od170-key="resumo-atributos"] [data-od170-toggle]')) {
      setTimeout(() => safe(() => typeof enforceAttributeCollapse === 'function' && enforceAttributeCollapse(), null), 0);
      return;
    }

    if (event.target.closest?.('.od170-module[data-od170-key="pericias-lista"] [data-od170-toggle]')) {
      event.preventDefault();
      event.stopImmediatePropagation();
      cleanupSkillsModule();
      return;
    ...
    if (event.target.closest?.('#sessions-menu-btn,.topbar-menu-toggle,#toggle-account-panel-btn,#sidebar-dock-btn')) {
      setTimeout(() => { cleanupSessionProfile(); refreshSheetButton(); }, 0);
      setTimeout(() => { cleanupSessionProfile(); refreshSheetButton(); }, 160);
    }
  }, true);
...
/* =========================
   V1812 - Compactação do hub e remoção total do card One Dice/Ficha Avulsa do menu
========================= */
(function od1812CompactHubAndRemoveProfileCard(){
  'use strict';
  ...
  function compactHub(){
    document.querySelector('#sessions-screen.active .od71-shell')?.classList.add('od1812-compact-shell');
    document.querySelector('#sessions-screen.active .od71-topbar')?.classList.add('od1812-compact-topbar');
    document.querySelector('#sessions-screen.active #od71-content')?.classList.add('od1812-compact-content');
    removeProfileCards();
  }

  document.addEventListener('click', event => {
    if (event.target.closest?.('#sessions-menu-btn,#toggle-account-panel-btn,#od71-settings-btn,[data-od71-tab],#back-to-sessions-btn')) {
      setTimeout(compactHub, 0);
      setTimeout(compactHub, 120);
    }
  }, true);

  if (document.readyState === 'loading') document.addEventListener('DOMContentLoaded', compactHub, { once: true });
  else compactHub();
  setTimeout(compactHub, 250);
  setTimeout(compactHub, 900);

  window.od1812CompactHubAndRemoveProfileCard = { compactHub, removeProfileCards };
})();


/* =========================
   V1813 - Cores do hub novo vinculadas ao tema
   Mantém o modo escuro, mas atualiza a cor de destaque do redesign
   conforme a cor escolhida na conta.
========================= */
(function od1813ThemeLinkedHub(){
  'use strict';
  ...
  const THEMES = {
    ...
    vermelho:{ main: '#ef2f3b', dark: '#9d1018', soft: 'rgba(239,47,59,.16)', line: 'rgba(239,47,59,.72)' },
    green:   { main: '#22c55e', dark: '#137a38', soft: 'rgba(34,197,94,.16)', line: 'rgba(34,197,94,.72)' },
    verde:   { main: '#22c55e', dark: '#137a38', soft: 'rgba(34,197,94,.16)', line: 'rgba(34,197,94,.72)' },
    blue:    { main: '#38bdf8', dark: '#075985', soft: 'rgba(56,189,248,.16)', line: 'rgba(56,189,248,.72)' },
    azul:    { main: '#38bdf8', dark: '#075985', soft: 'rgba(56,189,248,.16)', line: 'rgba(56,189,248,.72)' },
    gold:    { main: '#f5c542', dark: '#8a5a00', soft: 'rgba(245,197,66,.16)', line: 'rgba(245,197,66,.72)' },
    dourado: { main: '#f5c542', dark: '#8a5a00', soft: 'rgba(245,197,66,.16)', line: 'rgba(245,197,66,.72)' },
    purple:  { main: '#a855f7', dark: '#581c87', soft: 'rgba(168,85,247,.16)', line: 'rgba(168,85,247,.72)' },
    roxo:    { main: '#a855f7', dark: '#581c87', soft: 'rgba(168,85,247,.16)', line: 'rgba(168,85,247,.72)' },
    black:   { main: '#d1d5db', dark: '#374151', soft: 'rgba(209,213,219,.12)', line: 'rgba(209,213,219,.55)' },
    preto:   { main: '#d1d5db', dark: '#374151', soft: 'rgba(209,213,219,.12)', line: 'rgba(209,213,219,.55)' }
  };
  ...
  function readSettings(){
    ...
    safe(() => {
      if (typeof get === 'function' && typeof STORAGE !== 'undefined') {
        settings = get(STORAGE.settings, {}) || {};
      }
    }, null);

    if (!settings || !Object.keys(settings).length) {
      safe(() => { settings = JSON.parse(localStorage.getItem('one-dice-settings') || '{}') || {}; }, null);
    }
  ...
  function applyTheme(){
    ...
    const raw = settings.accent || settings.color || settings.themeColor || settings.accentColor || document.body?.dataset?.accent || 'red';
    const key = normalizeAccent(raw);
    const theme = THEMES[key] || THEMES.red;

    const root = document.documentElement;
    root.style.setProperty('--od-theme-main', theme.main);
    root.style.setProperty('--od-theme-dark', theme.dark);
    root.style.setProperty('--od-theme-soft', theme.soft);
    root.style.setProperty('--od-theme-line', theme.line);

    // Liga também no nome antigo usado por muitos blocos do site.
    root.style.setProperty('--accent', theme.main);
    root.style.setProperty('--accent-dark', theme.dark);
    root.style.setProperty('--accent-soft', theme.soft);

    document.body?.setAttribute('data-od1813-accent', key);
    root.setAttribute('data-od1813-accent', key);
  }

  document.addEventListener('change', event => {
    if (event.target.closest?.('#accent-select,#appearance-accent,[name="accent"],[data-accent-select]')) {
  ...
  document.addEventListener('click', event => {
    if (event.target.closest?.('#od84-theme-panel,#sessions-menu-btn,#od71-settings-btn,#theme-toggle,#sessions-theme-toggle')) {
      setTimeout(applyTheme, 0);
      setTimeout(applyTheme, 180);
    }
  }, true);
  ...
  window.od1813ThemeLinkedHub = { applyTheme };
})();


/* =========================
   V1815 - Rotas estáveis nas abas internas da ficha
   Corrige o salto rápido para /inicio ao trocar Resumo/Perícias/Combate/etc.
========================= */
(function od1815StableSheetTabsRoute(){
  'use strict';
  ...
  function isLocked(){
    return Date.now() < (window.__od1815SheetTabRouteLockUntil || 0);
  }

  function normalizeUrlArg(args){
    try {
      const raw = args && args.length >= 3 ? args[2] : '';
      if (!raw) return '';
  ...
  function shouldBlock(args){
    const path = normalizeUrlArg(args);
    if (!path) return false;
    if (!HUB_ROUTES.has(path)) return false;
    return isLocked() || isSheetOpen();
  }

  function lockSheetRoute(){
    window.__od1815SheetTabRouteLockUntil = Date.now() + 1000;
  }

  function installHistoryGuard(){
    if (history.pushState.__od1815Guarded) return;

    const basePush = history.pushState;
    const baseReplace = history.replaceState;

    history.pushState = function od1815PushStateGuarded(){
      if (shouldBlock(arguments)) return;
      return basePush.apply(this, arguments);
    };

    history.replaceState = function od1815ReplaceStateGuarded(){
      if (shouldBlock(arguments)) return;
      return baseReplace.apply(this, arguments);
    };

    history.pushState.__od1815Guarded = true;
    history.replaceState.__od1815Guarded = true;
  }
  ...
  document.addEventListener('keydown', event => {
    if ((event.key === 'Enter' || event.key === ' ') && event.target?.closest?.('.sheet-tab')) {
      lockSheetRoute();
    }
  }, true);
  ...
  window.od1815StableSheetTabsRoute = { lockSheetRoute, isLocked };
})();


/* =========================
   V1816 - Atributos clicáveis
   Clicar no quadrado do atributo rola automaticamente:
   - D20: 1D20 + bônus do atributo
   - Pool Dice: valor do atributo em D20, usando o maior resultado
========================= */
(function od1816ClickableAttributes(){
  'use strict';
  ...
  function attrValue(char, key){
    return Math.max(1, Number(char?.attrs?.[key] ?? 1) || 1);
  }
  function attrLabel(key){
    return (Array.isArray(window.ATTRIBUTE_KEYS) ? window.ATTRIBUTE_KEYS.find(a => a[0] === key)?.[1] : null)
      || ATTRS.find(a => a[0] === key)?.[1]
      || key;
  }
  function bonusFor(value){
    try { return typeof attrMod === 'function' ? attrMod(value) : Math.floor((Number(value) - 10) / 2); }
    catch (_) { return Math.floor((Number(value) - 10) / 2); }
  }
  function modelOf(char){
    try {
      if (window.od113SystemModel?.characterModel) return window.od113SystemModel.characterModel(char);
    } catch (_) {}
    const raw = String(
      char?.systemModel || char?.systemType || char?.model || char?.sheetModel ||
      char?.settings?.systemModel || char?.settings?.systemType || ''
    ).toLowerCase().replace(/[\s_-]+/g, '');
    return ['pool','pooldice','dados','ordem','ordemparanormal'].includes(raw) ? 'pool' : 'd20';
  }
  function showLast(text, html){
    const last = $('last-roll');
    if (!last) return;
  ...
  function rollPool(label, value){
    ...
    const text = `${label} • Pool Dice: ${qty}D20 → [${results.join(', ')}] melhor = ${best}`;
    showLast(text, `
      <div class="roll-result-card od1816-attr-roll-result">
        <div class="roll-result-title">${esc(label)} • Pool Dice</div>
        <div class="roll-result-main">
          <span>${esc(qty)}D20</span>
          <strong class="roll-result-total">${esc(best)}</strong>
        </div>
        <div class="roll-result-detail">Resultados: [${esc(results.join(', '))}] • maior resultado</div>
      </div>`);
    addRollChat(text);
    return { total: best, results };
  }
  function rollD20(label, value){
    const mod = bonusFor(value);
    if (typeof doRoll === 'function') return doRoll(`Teste de ${label}`, 1, 20, mod);
    const d20 = Math.floor(Math.random() * 20) + 1;
    const total = d20 + Number(mod || 0);
    const sign = Number(mod) >= 0 ? `+${Number(mod)}` : String(Number(mod));
    const text = `Teste de ${label}: 1D20${Number(mod) ? sign : ''} → [${d20}] = ${total}`;
    showLast(text);
    addRollChat(text);
    return { total, results: [d20] };
  }
  function rollAttribute(key){
    ...
    const value = attrValue(char, key);
    if (modelOf(char) === 'pool') rollPool(label, value);
    else rollD20(label, value);
  }
  function decorate(){
    document.querySelectorAll('.od1715-attr-summary .od1715-attr-mini').forEach((card, index) => {
      const key = card.dataset.od1816RollAttr || ATTRS[index]?.[0];
      if (!key) return;
      card.classList.add('od1816-attr-roll-target');
      card.dataset.od1816RollAttr = key;
      card.setAttribute('role', card.tagName === 'BUTTON' ? 'button' : 'button');
      card.setAttribute('tabindex', '0');
      card.setAttribute('title', `Rolar ${attrLabel(key)}`);
      card.setAttribute('aria-label', `Rolar ${attrLabel(key)}`);
    });

    document.querySelectorAll('#attributes-grid .attr-card-v2, #attributes-grid .od103-attr-card, #attributes-grid .od100-attr-card, #attributes-grid .od136-attr-card').forEach(card => {
      const key = card.querySelector('[data-roll-attr]')?.dataset?.rollAttr
        || card.querySelector('[data-od136-attr]')?.dataset?.od136Attr
        || card.querySelector('[data-attr]')?.dataset?.attr;
      if (!key) return;
      card.classList.add('od1816-attr-roll-target');
      card.dataset.od1816RollAttr = key;
      card.setAttribute('title', `Clique no card para rolar ${attrLabel(key)}`);
    });
  }

  document.addEventListener('click', event => {
    const card = event.target.closest?.('.od1816-attr-roll-target[data-od1816-roll-attr]');
    if (!card) return;

    // Evita rolar quando o usuário estiver editando valor ou usando botões internos.
    if (event.target.closest?.('input, textarea, select, option, [data-od100-attr-step], [data-od99-attr-step], [data-od98-attr-step], [data-od136-attr-step], .od100-step, .od99-step, .od98-attr-step, .od136-attr-step, .roll-attr')) return;

    event.preventDefault();
    event.stopPropagation();
    event.stopImmediatePropagation();
    rollAttribute(card.dataset.od1816RollAttr);
  }, true);
  ...
  document.addEventListener('click', event => {
    if (event.target.closest?.('[data-od170-toggle], .sheet-tab, #attributes-grid, .od1715-attr-summary')) {
  ...
  document.addEventListener('input', event => {
    if (event.target.closest?.('#attributes-grid input')) setTimeout(decorate, 120);
  }, true);
  ...
  window.od1816ClickableAttributes = { decorate, rollAttribute };
})();


/* =========================
   V1817 - Ataques clicáveis no modo reduzido
   Clicar no quadrado do ataque rola apenas o DANO do ataque.
========================= */
(function od1817ClickableAttackDamage(){
  'use strict';
  ...
  function parseDamageExpression(expr){
    ...
    while ((match = diceRegex.exec(clean))) {
      const sign = match[1] === '-' ? -1 : 1;
      const qty = Math.max(1, Number(match[2] || 1) || 1);
      const sides = Math.max(2, Number(match[3] || 20) || 20);
      parts.push({ type: 'dice', sign, qty, sides, text: `${sign < 0 ? '-' : ''}${qty}D${sides}` });
    ...
    let flat;
    let mod = 0;
    while ((flat = flatRegex.exec(consumed))) {
      mod += Number(flat[1] || 0);
    }

    if (!parts.length && !mod) {
      // fallback para o parser antigo, se existir
      try {
        if (typeof parseDamage === 'function') {
          const dmg = parseDamage(raw);
          if (dmg) return { raw, parts: [{ type: 'dice', sign: 1, qty: dmg.qty, sides: dmg.sides, text: `${dmg.qty}D${dmg.sides}` }], mod: Number(dmg.mod || 0) };
        }
      } catch (_) {}
      return null;
    }

    return { raw, parts, mod };
  }

  function rollDamageExpression(attack){
    const name = attack?.name || 'Ataque';
    const expr = attack?.damage || '';
    const parsed = parseDamageExpression(expr);

    if (!parsed) {
      const text = `Dano de ${name}: dano não configurado`;
      showLast(text);
      addRollChat(text);
      return null;
    }

    let total = Number(parsed.mod || 0);
    const detail = [];

    parsed.parts.forEach(part => {
      const results = Array.from({ length: part.qty }, () => Math.floor(Math.random() * part.sides) + 1);
      const sum = results.reduce((acc, value) => acc + value, 0) * part.sign;
      total += sum;
      detail.push(`${part.text} [${results.join(', ')}]${part.sign < 0 ? ` = ${sum}` : ''}`);
    });

    const modText = parsed.mod ? ` ${formatSigned(parsed.mod)}` : '';
    const notation = `${parsed.parts.map(p => p.text).join(' + ')}${modText}`.trim() || parsed.raw;
    const text = `Dano de ${name}: ${notation} → ${detail.join(' + ')}${parsed.mod ? ` ${formatSigned(parsed.mod)}` : ''} = ${total}`;

    showLast(text, `
      <div class="roll-result-card od1817-attack-roll-result">
        <div class="roll-result-title">Dano de ${esc(name)}</div>
        <div class="roll-result-main">
          <span>${esc(notation)}</span>
          <strong class="roll-result-total">${esc(total)}</strong>
        </div>
        <div class="roll-result-detail">${esc(detail.join(' + '))}${parsed.mod ? ` • Modificador ${esc(formatSigned(parsed.mod))}` : ''}</div>
      </div>`);
    addRollChat(text);
    return { total };
  }
  ...
  function decorate(){
    const char = current();
    const attacks = Array.isArray(char?.attacks) ? char.attacks : [];
    document.querySelectorAll('.od1715-attack-summary .od1715-attack-mini, .od1715-attack-mini').forEach((card, index) => {
      card.classList.add('od1817-attack-damage-roll');
      if (!card.dataset.od1817AttackIndex) card.dataset.od1817AttackIndex = String(index);
      const attack = attacks[Number(card.dataset.od1817AttackIndex || index)];
      card.setAttribute('role', card.tagName === 'BUTTON' ? 'button' : 'button');
      card.setAttribute('tabindex', '0');
      card.setAttribute('title', `Rolar dano de ${attack?.name || 'Ataque'}`);
      card.setAttribute('aria-label', `Rolar dano de ${attack?.name || 'Ataque'}`);
    });
  }

  document.addEventListener('click', event => {
    const card = event.target.closest?.('.od1817-attack-damage-roll');
    if (!card) return;

    // Não interfere no modo editável nem em botões internos.
    if (event.target.closest?.('input, textarea, select, button:not(.od1817-attack-damage-roll), .od80-card-order')) return;

    const char = current();
    const attacks = Array.isArray(char?.attacks) ? char.attacks : [];
    const attack = attacks[Number(card.dataset.od1817AttackIndex || 0)];
  ...
  document.addEventListener('click', event => {
    if (event.target.closest?.('[data-od170-toggle], .sheet-tab, #attacks-list')) {
      setTimeout(decorate, 0);
      setTimeout(decorate, 140);
    }
  }, true);
  ...
  document.addEventListener('input', event => {
    if (event.target.closest?.('#attacks-list input, #attacks-list textarea')) setTimeout(decorate, 160);
  }, true);
  ...
  window.od1817ClickableAttackDamage = { decorate, rollDamageExpression, parseDamageExpression };
})();


/* =========================
   V1818 - Habilidades clicáveis gastam custo
   Clicar no card reduzido da habilidade gasta automaticamente PE ou PV.
   Se não houver custo, não faz nada.
========================= */
(function od1818ClickableAbilityCost(){
  'use strict';
  ...
  function readAbilityCost(ability){
    const directAmount = num(ability?.costAmount, 0);
    let resource = String(ability?.costResource || '').toUpperCase().includes('PV') ? 'PV' : 'PE';

    if (directAmount > 0) return { amount: Math.max(0, directAmount), resource };

    const raw = String(ability?.cost || '').toUpperCase();
    const match = raw.match(/(\d+(?:[.,]\d+)?)\s*(PV|PE)/);
    if (!match) return { amount: 0, resource };

    return {
      amount: Math.max(0, num(match[1], 0)),
      resource: match[2] === 'PV' ? 'PV' : 'PE'
    };
  }
  function abilityFromCard(card){
    const all = [...document.querySelectorAll('.od1808-ability-card-view')];
    const index = all.indexOf(card);
    const c = char();
    const abilities = Array.isArray(c?.abilities) ? c.abilities : [];
    if (index >= 0 && abilities[index]) return { ability: abilities[index], index };

    // Fallback: lê os campos ocultos do card reduzido.
    return {
      index: Math.max(0, Number(card?.dataset?.index || 0) || 0),
      ability: {
        name: card?.querySelector('.ability-name')?.value || card?.querySelector('.od1808-view-title')?.textContent || 'Habilidade',
        costAmount: card?.querySelector('.ability-cost-amount')?.value || 0,
        costResource: card?.querySelector('.ability-cost-resource')?.value || 'PE',
        cost: card?.querySelector('.od1808-view-tags span')?.textContent?.replace(/^Custo:\s*/i, '') || '',
        bonus: card?.querySelector('.ability-bonus')?.value || '',
        action: card?.querySelector('.ability-action')?.value || '',
        description: card?.querySelector('.ability-description')?.value || ''
      }
  ...
  function syncResourceInputs(updated){
    const pv = $('pv-current');
    const pe = $('pe-current');
    if (pv) pv.value = updated?.pvCurrent ?? 0;
    if (pe) pe.value = updated?.peCurrent ?? 0;
  }
  function refreshAfterSpend(updated){
    syncResourceInputs(updated);
    safe(() => typeof updateBars === 'function' && updateBars(updated), null);
    safe(() => typeof updateOverlay === 'function' && updateOverlay(updated), null);
    safe(() => typeof renderCharacterList === 'function' && renderCharacterList(), null);
    safe(() => typeof od42ScheduleCharacterSave === 'function' && od42ScheduleCharacterSave(updated), null);
  }
  function spendAbilityCost(card){
    const c = char();
    if (!c || !card) return false;

    const { ability } = abilityFromCard(card);
    const { amount, resource } = readAbilityCost(ability);

    // Sem custo: não faz nada, como solicitado.
    ...
    safe(() => {
      if (typeof updateChar === 'function') {
        updateChar(saved => {
          const key = resource === 'PV' ? 'pvCurrent' : 'peCurrent';
          const currentValue = num(saved[key], 0);
          saved[key] = Math.max(0, currentValue - amount);
          updated = saved;
        });
      }
    }, null);

    updated = char() || updated || c;

    safe(() => {
      if (typeof addChat === 'function') addChat(`Usou ${ability?.name || 'Habilidade'}: -${amount} ${resource}.`, 'roll');
    }, null);

    refreshAfterSpend(updated);
    return true;
  }
  function decorate(){
    document.querySelectorAll('.od1808-ability-card-view').forEach((card, index) => {
      card.classList.add('od1818-ability-cost-click');
      card.dataset.od1818AbilityIndex = String(index);
      const title = card.querySelector('.od1808-view-title')?.textContent?.trim() || 'Habilidade';
      card.setAttribute('role', 'button');
      card.setAttribute('tabindex', '0');
      card.setAttribute('title', `Usar ${title}`);
      card.setAttribute('aria-label', `Usar ${title}`);
    });
  }

  document.addEventListener('click', event => {
    const card = event.target.closest?.('.od1818-ability-cost-click');
    if (!card) return;

    // Não interfere no modo editável nem em botões/campos internos.
    if (event.target.closest?.('input, textarea, select, option, button, .od80-card-order')) return;

    event.preventDefault();
    event.stopPropagation();
    event.stopImmediatePropagation();
    spendAbilityCost(card);
  }, true);
  ...
  document.addEventListener('click', event => {
    if (event.target.closest?.('.sheet-tab, [data-od170-toggle], #abilities-list')) {
      setTimeout(decorate, 0);
  ...
  window.od1818ClickableAbilityCost = { decorate, spendAbilityCost, readAbilityCost };
})();


/* =========================
   V1819 - Remoção final do card One Dice/Ficha Avulsa do menu da ficha
   Mantém o elemento técnico #current-user-label escondido para não quebrar códigos antigos.
========================= */
(function od1819RemoveFichaAvulsaMenuCard(){
  'use strict';
  ...
  function run(){
    ...
    if (brand) {
      brand.classList.add('od1819-removed-brand-card');
      brand.setAttribute('aria-hidden', 'true');
      brand.style.setProperty('display', 'none', 'important');
    }

    const content = topbar?.querySelector('.topbar-content');
    if (content) content.classList.add('od1819-no-brand-content');

    document.querySelectorAll(
      '.od108-info-card,.od108-panel-user-info,.od90-profile-card,[data-od90-profile-card],.session-profile-card,.account-profile-card,.menu-profile-card,#session-profile-card,#account-menu-profile,#sessions-menu-panel .od75-account-menu-card,#sessions-menu-panel .od90-user-menu-card'
    ).forEach(el => el.remove());
  }

  document.addEventListener('click', event => {
    if (event.target.closest?.('#topbar-menu-toggle,.topbar-menu-toggle,#sessions-menu-btn,#toggle-account-panel-btn,#sidebar-dock-btn,#back-to-sessions-btn,.sheet-tab')) {
      setTimeout(run, 0);
      setTimeout(run, 120);
      setTimeout(run, 350);
    }
  ...
  window.od1819RemoveFichaAvulsaMenuCard = { run };
})();


/* =========================
   V190 - Campanhas: portal narrativo redesenhado
========================= */
(function od190CampaignPortalRefresh(){
  'use strict';
  ...
  function isCampaigns(){
    const path = location.pathname.replace(/\/$/, '') || '/inicio';
    return path.includes('campanhas') || localStorage.getItem('od71_tab') === 'campaigns';
  }
  ...
  function refresh(){
    ...
    const content = document.getElementById('od71-content');
    if (content) {
      content.classList.remove('od1811-campaign-list', 'od86-campaign-list');
      content.classList.add('od190-campaign-shell');
      content.classList.remove('od86-campaigns-ready');
    }
    try { if (typeof od86RenderCampaigns === 'function') od86RenderCampaigns(true); } catch (_) {}
  }

  document.addEventListener('click', event => {
    if (event.target.closest?.('[data-od71-tab="campaigns"], [data-od75-tab="campaigns"], #od71-open-join, #od86-open-join, #od86-new-campaign, [data-od86-edit-campaign]')) {
  ...
  if (document.readyState === 'loading') document.addEventListener('DOMContentLoaded', () => setTimeout(refresh, 240), { once: true });
  else setTimeout(refresh, 240);

  window.od190CampaignPortalRefresh = { refresh };
})();


/* =========================
   V190.1 - Gerência interna de Campanhas/Mesa
   Novo fluxo independente do painel antigo da ficha.
========================= */
(function od1901CampaignManager(){
  'use strict';
  ...
  function campaignRole(member){ return String(member?.role || '').toLowerCase(); }
  function campaignIsMaster(member = currentMembership()){ return ['mestre','master','mestre_jogador','master_player'].includes(campaignRole(member)); }
  function memberCharacter(member){ return charById(member?.characterId); }
  ...
  function roleLabel(member){
    const role = campaignRole(member);
    if (['mestre','master'].includes(role)) return 'Mestre';
    if (['mestre_jogador','master_player'].includes(role)) return 'Mestre/Jogador';
    return 'Jogador';
  }
  function managerKey(campaignId = currentCampaignId){ return `od1901_campaign_tab_${campaignId || 'none'}`; }
  function activeTab(){
    const saved = localStorage.getItem(managerKey()) || 'personagens';
    const tab = TABS.includes(saved) ? saved : 'personagens';
    return tab === 'escudo' && !campaignIsMaster() ? 'personagens' : tab;
  }
  function setTab(tab){
  ...
  function ensureShell(){
    ...
    if (!shell) {
      ...
      shell.className = 'od1901-manager';
  ...
  function combatState(){
    return safe(() => getInitiativeState(), get(v35InitiativeKey(), { active: false, round: 1, entries: [] }));
  }
  function saveCombatState(state){
    safe(() => setInitiativeState(state), set(v35InitiativeKey(), state));
  }
  function modelOf(char){
  ...
  function rollInitiative(memberId){
    ...
    const char = memberCharacter(member);
    if (!member || !char) return;
    const mine = String(member.userId) === String(currentUser?.id);
    if (!campaignIsMaster() && !mine) return alert('Você só pode rolar iniciativa da sua ficha.');
    ...
    const payload = {
      id: existing?.id || uid('init'),
      characterId: char.id,
      userId: member.userId,
      memberId: member.id,
      name: char.name || 'Personagem',
      playerName: userDisplayName(userById(member.userId)),
      value: total,
      manual: false
    };
  ...
  function sendManagerChat(text){
    const clean = String(text || '').trim();
    if (!clean) return;
    const key = campaignChatKey();
    const chat = get(key, []);
    chat.push({ id: uid('msg'), user: userDisplayName(currentUser), text: clean, type: 'msg', at: nowTime() });
  ...
  function pageTabs(current, master){
    ...
    return base.map(([key,label]) => `<button type="button" class="${current === key ? 'active' : ''}" data-od1901-manager-tab="${key}">${label}</button>`).join('');
  }
  function hero(campaign, member, tab){
    const members = campaignMembers();
    const master = campaignIsMaster(member);
    return `
      <header class="od1901-hero">
        <div class="od1901-hero-top">
          <button type="button" class="od1901-back" data-od1901-back>← Voltar</button>
          <div class="od1901-hero-actions">
            ${master ? `<button type="button" data-od1901-add-characters>Adicionar Personagens</button><button type="button" data-od1901-edit-campaign>Editar Campanha</button>` : ''}
            <button type="button" data-od1901-copy-code="${esc(campaign.code || '')}">Copiar Código</button>
          </div>
        </div>
  ...
  function renderCharactersTab(){
    const master = campaignIsMaster();
    const members = campaignMembers();
    const rows = members.map(member => {
      const char = memberCharacter(member);
      const user = userById(member.userId);
      if (!char) {
        return `<article class="od1901-character-row empty"><div><strong>${esc(userDisplayName(user))}</strong><small>Sem personagem vinculado</small></div></article>`;
      }
      return `<article class="od1901-character-row">
        ...
        <div class="od1901-character-actions">
          ${(master || String(member.userId) === String(currentUser?.id)) ? `<button type="button" data-od1901-open-sheet="${esc(char.id)}">Acessar Ficha</button>` : ''}
          ${master ? `<button type="button" data-od1901-open-obs="${esc(char.id)}">OBS</button>` : ''}
          ${master && String(member.userId) !== String(currentUser?.id) ? `<button type="button" class="danger" data-od1901-remove-member="${esc(member.id)}">Remover</button>` : ''}
        </div>
      </article>`;
    }).join('');
    return `<section class="od1901-panel"><div class="od1901-section-head"><div><h2>Personagens da Campanha</h2><p>Nomes, jogadores, entrada na mesa e ações rápidas do mestre.</p></div></div><div class="od1901-list">${rows || '<div class="od1901-empty">Nenhum personagem vinculado ainda.</div>'}</div></section>`;
  }
  function renderCombatTab(){
    const master = campaignIsMaster();
    const members = campaignMembers().filter(m => memberCharacter(m));
    const state = combatState();
    const entries = [...(state.entries || [])].sort((a,b) => Number(b.value || 0) - Number(a.value || 0));
    const participants = members.map(member => {
      ...
      return `<article class="od1901-combat-participant">
        <img src="${esc(char.portrait || 'assets/logo.jpg')}" alt="" />
        <div><strong>${esc(char.name || 'Personagem')}</strong><small>${esc(userDisplayName(user))} • AGI ${esc(char.attrs?.agilidade ?? 10)}</small></div>
        <button type="button" ${canRoll ? '' : 'disabled'} data-od1901-roll-init="${esc(member.id)}">Rolar Iniciativa</button>
      </article>`;
    }).join('');
    ...
    return `<section class="od1901-panel od1901-combat">
      <div class="od1901-section-head">
        <div><h2>Combate</h2><p>Jogadores rolam iniciativa e a ordem é organizada automaticamente.</p></div>
        <div class="od1901-toolbar">
          ${master ? `<button type="button" data-od1901-toggle-combat>${state.active ? 'Encerrar Combate' : 'Iniciar Combate'}</button><button type="button" data-od1901-clear-init>Limpar Ordem</button>` : ''}
        </div>
      </div>
      <div class="od1901-combat-grid">
        <div><h3>Participantes</h3><div class="od1901-list compact">${participants || '<div class="od1901-empty">Nenhum personagem para combate.</div>'}</div></div>
        <div><h3>Ordem de Turno</h3><div class="od1901-init-list">${order || '<div class="od1901-empty">Aguardando rolagens de iniciativa.</div>'}</div></div>
      </div>
    </section>`;
  }
  function renderPlayersTab(){
    const rows = campaignMembers().map(member => {
      ...
      return `<article class="od1901-player-row">
        <div class="od1901-avatar">${esc((userDisplayName(user) || '?').slice(0,1).toUpperCase())}</div>
        <div><strong>${esc(userDisplayName(user))}</strong><small>${esc(roleLabel(member))} • ${esc(entryDate(member))}</small></div>
        <span class="${online ? 'online' : 'offline'}">${online ? 'Online agora' : 'Offline'}</span>
      </article>`;
    }).join('');
    return `<section class="od1901-panel"><div class="od1901-section-head"><div><h2>Jogadores</h2><p>Contas vinculadas, função e status de presença.</p></div></div><div class="od1901-list">${rows || '<div class="od1901-empty">Nenhum jogador na campanha.</div>'}</div></section>`;
  }
  function renderChatTab(){
    ...
    return `<section class="od1901-panel od1901-chat-panel">
      <div class="od1901-section-head"><div><h2>Chat ao Vivo</h2><p>Mensagens dos jogadores dentro da campanha.</p></div></div>
      <div id="od1901-chat-log" class="od1901-chat-log">${html || '<div class="od1901-empty">Nenhuma mensagem ainda.</div>'}</div>
      <form id="od1901-chat-form" class="od1901-chat-form"><input id="od1901-chat-input" placeholder="Mensagem para a campanha..." autocomplete="off" /><button>Enviar</button></form>
    </section>`;
  }
  function renderShieldTab(){
    if (!campaignIsMaster()) return `<div class="od1901-empty">Escudo disponível apenas para o mestre.</div>`;
    const cards = campaignMembers().map(member => {
      const char = memberCharacter(member);
      const user = userById(member.userId);
      if (!char) return '';
      const pvCur = Number(char.pvCurrent || 0), pvMax = Number(char.pvMax || 0), peCur = Number(char.peCurrent || 0), peMax = Number(char.peMax || 0);
      const pvPct = pvMax ? Math.max(0, Math.min(100, (pvCur / pvMax) * 100)) : 0;
      const pePct = peMax ? Math.max(0, Math.min(100, (peCur / peMax) * 100)) : 0;
      return `<article class="od1901-shield-card">
        <img src="${esc(char.portrait || 'assets/logo.jpg')}" alt="" />
        <div class="od1901-shield-body">
          <h3>${esc(char.name || 'Personagem')}</h3>
          <small>${esc(userDisplayName(user))}</small>
          <label>PV <span>${esc(pvCur)} / ${esc(pvMax)}</span><i><b style="width:${pvPct}%"></b></i></label>
          <label>PE <span>${esc(peCur)} / ${esc(peMax)}</span><i><b style="width:${pePct}%"></b></i></label>
        </div>
        <button type="button" data-od1901-open-sheet="${esc(char.id)}">Ficha</button>
      </article>`;
    }).join('');
    return `<section class="od1901-panel"><div class="od1901-section-head"><div><h2>Escudo do Mestre</h2><p>PV, PE, foto e nome dos personagens para consulta rápida.</p></div></div><div class="od1901-shield-grid">${cards || '<div class="od1901-empty">Nenhuma ficha vinculada.</div>'}</div></section>`;
  }
  ...
  function renderManager(){
    ...
    const chat = $('od1901-chat-log');
    if (chat) chat.scrollTop = chat.scrollHeight;
  }
  function injectSheetInitiativeButton(){
    if (!currentCampaignId || accountSheetMode) return;
    const tabs = document.querySelector('.sheet .tabs');
    if (!tabs || $('od1901-sheet-init-btn')) return;
    const btn = document.createElement('button');
    btn.id = 'od1901-sheet-init-btn';
    btn.type = 'button';
    btn.className = 'sheet-tab od1901-sheet-init-btn';
  ...
  enterCampaign = function(campaignId){
    return openManager(campaignId || currentCampaignId);
  };

  initApp = function(campaignId = currentCampaignId){
    if (window.__od1901OpeningLegacySheet || accountSheetMode) return legacyInitApp?.(campaignId);
    if (campaignId && getMembers().some(m => String(m.campaignId) === String(campaignId) && String(m.userId) === String(currentUser?.id))) {
      return openManager(campaignId);
    }
    return legacyInitApp?.(campaignId);
  };

  showSessions = function(){
    setManagerMode(false);
    return legacyShowSessions?.();
  };

  document.addEventListener('click', event => {
    const tab = event.target.closest?.('[data-od1901-manager-tab]');
    if (tab) { event.preventDefault(); return setTab(tab.dataset.od1901ManagerTab); }

    const back = event.target.closest?.('[data-od1901-back]');
    if (back) { event.preventDefault(); return showSessions(); }

    const add = event.target.closest?.('[data-od1901-add-characters]');
    if (add) { event.preventDefault(); return openChooseCharacterModal(currentCampaignId); }

    const open = event.target.closest?.('[data-od1901-open-sheet]');
    if (open) { event.preventDefault(); return openSheet(open.dataset.od1901OpenSheet); }

    const obs = event.target.closest?.('[data-od1901-open-obs]');
    if (obs) { event.preventDefault(); return openObs(obs.dataset.od1901OpenObs); }

    const remove = event.target.closest?.('[data-od1901-remove-member]');
    if (remove) { event.preventDefault(); return removeMember(remove.dataset.od1901RemoveMember); }

    const roll = event.target.closest?.('[data-od1901-roll-init]');
    if (roll) { event.preventDefault(); return rollInitiative(roll.dataset.od1901RollInit); }

    const toggle = event.target.closest?.('[data-od1901-toggle-combat]');
    if (toggle) { event.preventDefault(); return toggleCombat(); }

    const clear = event.target.closest?.('[data-od1901-clear-init]');
    if (clear) { event.preventDefault(); return clearInitiative(); }

    const copy = event.target.closest?.('[data-od1901-copy-code]');
    if (copy) { event.preventDefault(); return copyCode(copy.dataset.od1901CopyCode); }

    const edit = event.target.closest?.('[data-od1901-edit-campaign]');
    if (edit) { event.preventDefault(); return alert('Editor completo de campanha ficará na próxima etapa. Esta versão entrega a gerência interna sem camadas antigas.'); }
  }, true);

  document.addEventListener('submit', event => {
    const form = event.target.closest?.('#od1901-chat-form');
    if (!form) return;
    event.preventDefault();
    const input = $('od1901-chat-input');
    sendManagerChat(input?.value || '');
    if (input) input.value = '';
  }, true);

  document.addEventListener('DOMContentLoaded', () => {
    const active = safe(() => getSessionValue(), null) && safe(() => JSON.parse(localStorage.getItem(STORAGE.activeCampaign) || 'null'), null);
    if (active && getMembers().some(m => String(m.campaignId) === String(active) && String(m.userId) === String(currentUser?.id))) {
      setTimeout(() => openManager(active), 250);
    }
  }, { once: true });
